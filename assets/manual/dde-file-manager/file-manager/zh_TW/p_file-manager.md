...
當目錄下檔案數量龐大時，可以透過分組功能對檔案或資料夾進行分類整理，以便快速定位特定類型檔案。

1. 在工作區空白處單擊滑鼠右鍵，在選單中選擇 **分組方式**。您也可以在標題列的排序選項下拉選單底部，單擊選擇 **分組方式**。
2. 在選項中，選擇按照 **修改時間**、**建立時間**、**名稱**、**大小** 或 **類型** 進行分組展示。選擇 **無** 可取消分組模式。
3. 分組折疊與展開：在開啟分組後，單擊各分組頂部標籤內的展開折疊按鈕即可展開或折疊該分組。
...
若需徹底刪除檔案以防資料復原，可使用檔案粉碎功能保障安全。

1. 單擊 **主選單** > **設定**，進入 **進階設定**，在檔案粉碎模組中開啟 **開啟檔案粉碎功能** 開關。
2. 在檔案管理器中，右鍵單擊想要徹底刪除的檔案或資料夾。
3. 選擇 **檔案粉碎**。
4. 在跳出的確認粉碎視窗中，單擊 **確定** 按鈕執行粉碎過程。

> ![attention](../common/attention.svg) 注意: 使用此功能後，檔案將被徹底刪除且任何工具都不能復原，請謹慎操作。
...
使用快速鍵 **Ctrl + F** 或在網址列中點擊搜尋按鈕進入搜尋狀態，輸入關鍵字後將立即開始檔案搜尋，尋找相關檔案。

![](fig/search_button.png)

檔案管理員支援多種檢索方式，既支援透過檔案名稱、檔案內容進行一般搜尋，也支援透過檔案類型、建立時間等進行進階搜尋，提高工作效率，便於檔案管理。

- 指定目錄搜尋時，請先進入該目錄，然後再進行搜尋。
- 若您想搜尋多個關鍵字，可以在搜尋方塊中輸入多個詞語並用空格分隔。
- 檔案搜尋預設使用檔案索引，若您手動關閉索引，檔案搜尋將遍歷檔案系統，嚴重影響搜尋速度與搜尋功能。
...
1. 在檔案管理員中，點擊 ![icon_menu](../common/icon_menu.svg) > **設定**。
2. 在「**進階設定 -> 搜尋**」中勾選「**全文搜尋**」來開啟此功能。
...
1. 在檔案管理員中，點擊 ![icon_menu](../common/icon_menu.svg) > **設定**。
2. 在「**進階設定 -> 搜尋**」中勾選「**圖片文字內容搜尋**」來開啟此功能。



#### 進階搜尋

當檔案較多，搜尋較慢時，啟用進階搜尋縮小搜尋範圍，提高搜尋效率。

1. 在搜尋狀態下，輸入關鍵字後按 **Enter** 鍵，當搜尋到結果時，搜尋方塊右側顯示圖示 ![filter](../common/filter.svg)，點擊該圖示可進行進階搜尋的操作。
2. 選擇搜尋範圍、檔案大小、檔案類型、修改時間、存取時間與建立時間，進行更精準的搜尋。
...
對於需長期高頻訪問的固定目錄（如程式碼庫、報表資料夾等），您可以使用分頁固定功能將其常駐顯示。

- **固定分頁**：右鍵單擊需要固定的普通分頁，選擇 **固定分頁** 。固定後的分頁會縮小顯示區域並鎖定在分頁列。
- **取消固定**：右鍵單擊已固定的分頁，選擇 **取消固定分頁**，或者直接單擊固定分頁上的「圖釘」圖示取消固定。
- **管理固定分頁**：支援左右拖曳調整固定分頁的排列順序。您也可以將固定分頁直接拖出以形成全新的檔案管理器視窗，或拖入其他現有的視窗中作為普通分頁合併。
...
您也可以為網路共享的電腦IP目錄設定備註名：

1. 在電腦的磁碟清單中，或者在側邊欄的「分割區」及「網路」分組下，找到已連線的網路共享目錄。
2. 右鍵單擊該目錄，選擇 **重新命名**（或選中後緩慢雙擊圖示），使名稱進入全選輸入狀態。
3. 輸入新的備註名稱後，按 **Enter** 鍵或單擊其他空白區域儲存。
...
1. 單擊導航欄中的保險箱圖示，或者在計算機界面雙擊 “保險箱”，在彈出的對話框中單擊 **開啟**。 
...
將文件放入保險箱後，右鍵單擊導航欄圖示或者在電腦介面右鍵單擊「保險箱」。
...
> ![0|tips](../common/tips.svg)竅門：您也可以右鍵單擊或者 **保險箱**，選擇 **解鎖**，解鎖保險箱。

### 刪除保險箱

刪除保險箱需要提前解鎖保險箱。

1. 右鍵單擊導航欄圖示或者在電腦介面右鍵單擊 「保險箱」，選擇 **刪除保險箱**。
...
- **開啟行為**：設定是否 **總是在新視窗中開啟資料夾**、**在新分頁中開啟新資料夾**，以及設定 **雙擊** 或 **單擊** 開啟檔案。同時可以設定當重複開啟相同資料夾時，是否 **重複開啟資料夾時啟用已有視窗**。
- **新視窗**：設定 **啟動新視窗時顯示** 的目錄範圍。除「電腦」、「主目錄」、「文件」等系統的預設目錄外，還可以單擊 **新增目錄** 自訂補充最多4個啟動目錄。
- **新分頁**：設定檔案管理器的 **從新分頁開啟** 指定目錄。
...
- **搜尋**：設定 **檔案索引**、**全文搜尋**、**圖片文字內容搜尋** 索引開關。檔案索引自動更新，點擊 **立即更新索引** 可以手動重新整理。
- **掛載**：自動掛載外接裝置、掛載後開啟或顯示 Samba 項目。
- **外接儲存裝置使用模式**：選擇接入外接裝置的存取策略，可選 **即時同步安全拔除（推薦）** 或 **效能優先（需安全移除）** 以符合個人的效能需求和插拔習慣。
- **對話框**：使用檔案管理員選取檔案或啟用刪除確認。
- **檔案傳送**：設定分享權限及儲存位置。
- **檔案粉碎**：勾選 **開啟檔案粉碎功能** 後，可以從檔案或資料夾的右鍵選單中啟用「檔案粉碎」，以徹底清除不需保留的隱私敏感檔案。
