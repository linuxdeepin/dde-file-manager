...
当目录下文件数量庞大时，可以通过分组功能对文件或文件夹进行分类整理，以便快速定位特定类型文件。

1. 在工作区空白处单击鼠标右键，在菜单中选择 **分组方式** 。您也可以在标题栏的排序选项下拉菜单底部，单击选择 **分组方式**。
2. 在选项中，选择按照 **修改时间**、**创建时间**、**名称**、**大小** 或 **类型** 进行分组展示。选择 **无** 可取消分组模式。
2. 分组折叠与展开：在开启分组后，单击各分组顶部标签内的展开折叠按钮即可展开或折叠该分组。

![0|open](fig/group_files.png)
...
若需彻底删除文件以防数据恢复，可使用文件粉碎功能保障安全。

1. 单击 **主菜单** > **设置**，进入 **高级设置**，在文件粉碎模块中开启 **开启文件粉碎功能** 开关。
2. 在文件管理器中，右键单击想要彻底删除的文件或文件夹。
3. 选择 **文件粉碎**。
4. 在弹出的确认粉碎窗口中，单击 **确定** 按钮执行粉碎过程。

> ![attention](../common/attention.svg) 注意：使用此功能后，文件将被彻底删除且任何工具都不能恢复，请谨慎操作。
...
- 文件搜索默认使用文件索引，如果您手动关闭索引，文件搜索将遍历文件系统，严重影响搜索速度和搜索功能。
...
1. 在文件管理器中，单击 ![icon_menu](../common/icon_menu.svg) > **设置**。
2. 在“高级设置->搜索”中勾选“图片文本内容搜索”来开启此功能。
...
对于需长期高频访问的固定目录（如代码库、报表文件夹等），您可以使用标签页固定功能将其常驻显示。

- **固定标签页**：右键单击需要固定的普通标签页，选择 **固定标签页** 。固定后的标签页会缩小显示区域并锁定在标签页栏。
- **取消固定**：右键单击已固定的标签页，选择 **取消固定标签页**，或者直接单击固定标签页上的“图钉”图标取消固定。
- **管理固定标签**：支持左右拖拽调整固定标签页的排列顺序。您也可以将固定标签页直接拖出以形成全新的文件管理器窗口，或拖入其他现有的窗口中作为普通标签页合入。
...
您也可以为网络共享的电脑IP目录设置备注名：

1. 在计算机的磁盘列表中，或者在侧边栏的“分区”及“网络”分组下，找到已连接的网络共享目录。
2. 右键单击该目录，选择 **重命名**（或选中后缓慢双击图标），使名称进入全选输入状态。
3. 输入新的备注名称后，按 **Enter** 键或单击其他空白区域保存。
...
1. 在终端输入命令 **gio set xxx(文件路径)  -t stringv metadata::emblems "xxx(图标路径)"** ，该命令默认在文件（夹）右下角添加角标。
   ...
   > ![attention](../common/attention.svg) 注意：系统已定义的特殊角标，不支持被替换。
...
1. 单击导航栏中的保险箱图标，或者在计算机界面双击 “保险箱”，在弹出的对话框中单击 **开启**。 
...
3. 在恢复密钥文件窗口中，设置密钥文件的存储位置，单击 **下一步**。如果您忘记了保险箱密码，可使用密钥文件找回密码。
...
> ![tips](../common/tips.svg) 窍门：您也可以右键单击 **保险箱**，选择 **解锁**，解锁保险箱。
...
2. 弹出“连接到服务器”窗口，在协议下拉框中，选择一种连接协议；在地址栏输入服务器IP地址。
   ...
   > ![tips](../common/tips.svg) 窍门：您可以单击 ![icon](../common/collect.svg) ，将常用服务器地址添加到“我收藏的服务器”列表中。
...
设置是否 **总是在新窗口中打开文件夹**、**在新标签页中打开新文件夹**，以及设定 **双击** 或 **单击** 打开文件。同时可以设置当重复打开相同文件夹时，是否 **重复打开文件夹时激活已有窗口**。

**新窗口**

设置 **启动新窗口时显示** 的目录范围。除“计算机”、“主目录”、“文档”等系统的默认目录外，还可以单击 **添加目录** 自定义补充最多4个启动目录。

**新标签页**

设置文件管理器的 **从新标签页打开** 指定目录。
...
- 设置 **文件索引**、**全文检索**、**图片文本内容搜索** 索引开关。文件索引自动更新，单击 **立即更新索引** 可以手动刷新。
...
- **外部存储设备使用模式**：选择接入外部设备的存取策略，可选 **实时同步安全拔盘（推荐）** 或 **性能优先（需安全移除）** 以匹配个人的性能需要和插拔习惯。
...
- **开启文件粉碎功能**：勾选后，可以从文件或文件夹的右键菜单中启用“文件粉碎”，以彻底清除不需留存的隐私敏感文件。
