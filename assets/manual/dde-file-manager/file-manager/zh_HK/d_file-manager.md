...
> ![tips](../common/tips.svg) 貼士：拖曳左側導覽列右邊分隔線調整寬度。
...
若需徹底刪除檔案以防數據恢復，可使用檔案粉碎功能保障安全。

1. 單擊 **主菜單** > **設置**，進入 **高級設置**，在檔案粉碎模塊中開啟 **開啟檔案粉碎功能** 開關。
2. 在檔案管理員中，右鍵單擊想要徹底刪除的檔案或資料夾。
3. 選擇 **檔案粉碎**。
4. 在彈出的確認粉碎窗口中，單擊 **確定** 按鈕執行粉碎過程。

> ![attention](../common/attention.svg) 注意: 使用此功能後，檔案將被徹底刪除且任何工具都不能恢復，請謹慎操作。
...
使用快捷鍵 **Ctrl + F** 或在網址列中按一下搜尋按鈕進入搜尋狀態，輸入關鍵字後將立即開始檔案搜尋，尋找相關檔案。
...
- 如果您想搜尋多個關鍵字，可以在搜尋方塊中輸入多個詞語並用空格分隔。
- 檔案搜尋預設使用檔案索引，如果您手動關閉索引，檔案搜尋將遍歷檔案系統，嚴重影響搜尋速度和搜尋功能。
...
1. 在檔案管理員中，按一下 ![icon_menu](../common/icon_menu.svg) > **設定**。
...
對於需長期高頻訪問的固定目錄（如代碼庫、報表資料夾等），您可以使用分頁固定功能將其常駐顯示。

- **固定分頁**：右鍵單擊需要固定的普通分頁，選擇 **固定分頁** 。固定後的分頁會縮小顯示區域並鎖定在分頁欄。
- **取消固定**：右鍵單擊已固定的分頁，選擇 **取消固定分頁**，或者直接單擊固定分頁上的「圖釘」圖標取消固定。
- **管理固定分頁**：支持左右拖拽調整固定分頁的排列順序。您也可以將固定分頁直接拖出以形成全新的檔案管理員窗口，或拖入其他現有的窗口中作為普通分頁合入。
...
> ![tips](../common/tips.svg) 竅門：單擊網絡路徑和用戶名右側的複製按鈕 ![icon](../common/copy.svg)， 將訪問路徑和用戶名複製到剪貼板，便於您將訊息分享給訪問者。
...
您也可以為網絡共享的電腦IP目錄設置備註名：

1. 在計算機的磁盤列表中，或者在側邊欄的「分區」及「網絡」分組下，找到已連接的網絡共享目錄。
2. 右鍵單擊該目錄，選擇 **重命名**（或選中後緩慢雙擊圖標），使名稱進入全選輸入狀態。
3. 輸入新的備註名稱後，按 **Enter** 鍵或單擊其他空白區域保存。
...
- **開啟行為**：設置是否 **總是在新窗口中開啟資料夾**、**在新分頁中開啟新資料夾**，以及設定 **雙擊** 或 **單擊** 開啟檔案。同時可以設置當重複開啟相同資料夾時，是否 **重複開啟資料夾時激活已有窗口**。
- **新窗口**：設置 **啟動新窗口時顯示** 的目錄範圍。除「計算機」、「主目錄」、「文檔」等系統的默認目錄外，還可以單擊 **添加目錄** 自定義補充最多4個啟動目錄。
- **新分頁**：設置檔案管理員的 **從新分頁開啟** 指定目錄。
...
- **搜尋**：設定 **檔案索引**、**全文搜尋**、**圖片文字內容搜尋** 索引開關。檔案索引自動更新，按一下 **立即更新索引** 可以手動重整。
- **掛載**：自動掛載外置磁碟、掛載後開啟、顯示 Samba 項目。
- **外部存儲設備使用模式**：選擇接入外部設備的存取策略，可選 **實時同步安全拔除（推薦）** 或 **性能優先（需安全移除）** 以匹配個人的性能需要和插拔習慣。
- **對話框**：使用檔案管理員選擇檔案或啟用刪除確認。
- **檔案傳送**：設定共享權限及儲存位置。
- **檔案粉碎**：勾選 **開啟檔案粉碎功能** 後，可以從檔案或資料夾的右鍵菜單中啟用「檔案粉碎」，以徹底清除不需留存的私隱敏感檔案。
