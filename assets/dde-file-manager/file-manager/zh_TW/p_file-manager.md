...
> ![notes](../common/notes.svg)說明：直接雙擊文件會使用預設程式打開。您可以右鍵單擊文件，在 **屬性** > **其他程式開啟** 中選擇一個預設程式，也可以到控制中心 [預設程式](dman:///dde#預設程式設定) 中去設定此類文件的全域預設程式。

![0|open](fig/open.png)

> ![tips](../common/tips.svg)竅門：您可以選擇相同類型的多個文件，右鍵選擇「用其他程式開啟」選項，一次開啟多個文件。
...
在主選單中，您可以新增視窗、切換視窗主題、連線到伺服器、[設定共享密碼](#共享本機文件)、設定檔案管理器、查看說明手冊和版本介紹。
