...
統信UOS助手是一款實現手機與電腦間跨端協作、高效傳輸以及即時同步的工具，將手機端與電腦端連接同一無線網路，便可以輕鬆傳輸圖片、影片、文件等。

>![icon](../common/notes.svg) 說明：僅安卓手機端支援安裝統信UOS助手。

### 安裝統信UOS助手

1. 單擊左側導航欄 **我的手機** 或系統工具列中的圖示 ![icon](../common/phone.svg)。
...
前提條件：請先將手機端與電腦端連接同一無線網路且在同一個網段內，網路連線成功後，再執行如下步驟。

1. 單擊左側導航欄 **我的手機** 或系統工具列中的圖示 ![icon](../common/phone.svg)。
2. 在我的手機介面，單擊 **掃碼連接**，彈出二維碼視窗。

   ![phone](fig/myphone1.png)

3. 打開手機端統信UOS助手，單擊 **掃碼連接**，掃描步驟2中的二維碼，連線成功後即可進行文件傳輸。

如果需要斷開連接，您可以：
- 在統信UOS助手介面上單擊按鈕 ![icon](../common/exit.svg) 斷開連接。
- 在電腦上右鍵單擊系統工具列中的圖示 ![icon](../common/phone.svg)，選擇您的手機，在跳出視窗中單擊 **斷開連接**。
...
在統信UOS助手介面，單擊 **傳輸助手**，發送文件至電腦端或接收來自電腦端的文件。

在 **我的電腦** 介面查看、下載或分享電腦文件。

#### 電腦端

單擊左側導航欄 **我的手機**，分類展示手機相冊、影片、文件等資料夾，您可以打開、編輯資料夾中的文件，文件經過編輯並儲存後將同步至手機端。右側傳輸記錄區域，可以將電腦端的文件發送至手機，或接收來自手機端的文件。
