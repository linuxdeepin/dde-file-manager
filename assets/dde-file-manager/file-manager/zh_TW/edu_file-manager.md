...
![1|main-interface](fig/edu_maininterface.png)
...
![0|contextmenu](fig/edu_rename.png)
...
![1|rename](fig/edu_batchrename.png)
...
![1|icon-view](fig/edu_iconview.png)

- 列表檢視：列表顯示文件圖示或縮圖、名稱、修改時間、大小、類型等訊息。

![1|list-view](fig/edu_listview.png)

> ![tips](../common/tips.svg) 竅門：
> - 在列表檢視中，把游標置於兩列之間的分隔線上，拖動它來改變列的寬度；雙擊分隔線可將目前列自動調整為本列內容最寬的寬度。
> - 使用  **Ctrl + 1**  和  **Ctrl + 2**   快捷鍵，切換圖示檢視和列表檢視。
...
> ![notes](../common/notes.svg) 說明：若想隱藏「最近」，可在 **設定 > 基本設定 > 隱藏文件**，取消勾選「在左方面板顯示最近檔案項目」。若要隱藏某一個文件的訪問記錄，右擊該文件選擇 **移除**，該操作不會刪除文件。
...
![1|tag info](fig/edu_taginfo.png)
...
![1|color tag](fig/edu_colortag.png)

##### 透過右側訊息欄添加標記
選擇檔案（資料夾）後，單擊 ![fileinfo](../common/fileinfo.svg)，在右側訊息欄中也可以添加標記。

![1|info tag](fig/edu_infotag.png)
...
1. 在檔案管理器介面空白處單擊滑鼠右鍵。
2. 選擇 **以管理員身份打開**，在彈出的對話框中輸入使用者密碼，點擊 **確定**。
...
![0|open-terminal](fig/edu_openinterminal.png)
...
統信UOS助手是一款實現手機與電腦間跨端協作、高效傳輸的實用工具，只需要將手機與電腦連接同一網路，便可以輕鬆傳輸圖片、影片、文件等，同時也支援線上編輯手機文件，即時同步儲存。

### 安裝統信UOS助手

1. 單擊系統工具列中的圖示 ![icon](../common/phone.svg) 或打開檔案管理器，單擊左側導航欄 **我的手機**。
2. 單擊 **安裝統信UOS助手**，彈出二維碼視窗。
   ![phone](fig/myphone.png)

3. 打開手機，透過微信或瀏覽器掃描跳出視窗中的二維碼，下載並安裝統信UOS助手；您也可以單擊跳出視窗中的按鈕 **下載APK安裝包**，將安裝包下載到本機，再匯入至手機中進行安裝。

   >![icon](../common/notes.svg) 說明：僅支援安卓用戶端。

### 連接手機與電腦

請先將手機和電腦連接同一網路，桌上型電腦需要提前插入無線網卡。
...
3. 打開手機中的統信UOS助手，單擊 **掃碼連接**，掃描電腦視窗中的二維碼，連線成功後便可以進行文件傳輸和管理。

   

>![icon](../common/notes.svg) 說明：在統信UOS助手中單擊按鈕 ![icon](../common/exit.svg) 斷開連接，或在電腦上右鍵單擊系統工具列中的圖示 ![icon](../common/phone.svg)，選擇您的手機，在彈出的視窗中單擊 **斷開連接**。

### 文件傳輸

手機與電腦連線成功後，便可以互傳文件，輕鬆管理手機中的各類文件。

#### 在電腦上管理手機資料夾 

在 **檔案管理器 > 我的手機** 介面，手機資料夾會分類展示，您可以查看、管理手機中的檔案，也可以在電腦中編輯手機檔案，並直接儲存至手機；右側傳輸記錄區域，支援將電腦中的檔案發送至手機，或接收手機檔案。單擊右上角按鈕 ![icon](../common/more.svg) 查看最近檔案或清空記錄。

![pic](fig/phone_interface.png)

#### 在手機端查看電腦檔案

手機端不僅可以傳輸檔案至電腦，還可以查看電腦檔案。在手機中打開統信UOS助手，您可以：

- 在 **傳輸助手** 介面，發送手機檔案至電腦或接收電腦檔案，單擊右上角按鈕 ![icon](../common/more.svg) 查看最近檔案或清空記錄。

- 在 **我的電腦** 介面，查看、下載或分享電腦檔案。
