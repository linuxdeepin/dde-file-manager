...
2. 上下滾動鼠標滾輪瀏覽或通過搜索找到檔案管理員 ![deepin_file_manager](../common/deepin_file_manager.svg)，單擊運行。
...
在檔案管理員界面上，按下**Ctrl + Shift + /**  組合鍵來查看快捷鍵，熟練地使用快捷鍵，將大大提升您的操作效率。
...
單擊界面上的 ![icon_view](../common/icon_view.svg) 、![list_view](../common/list_view.svg) 圖標來切換圖標視圖和列表視圖。
...
> ![notes](../common/notes.svg)說明：直接雙擊文件會使用默認程序打開。您可以右鍵單擊文件，在 **屬性** > **打開方式** 中選擇一個默認程序，也可以到控制中心 [默認程序](dman:///dde#默認程序設置) 中去設置此類文件的全局默認程序。

![0|open](fig/open.png)

> ![tips](../common/tips.svg)竅門：您可以選擇相同類型的多個文件，單擊右鍵選擇「打開方式」選項，一次打開多個文件。
...
1. 在檔案管理員界面上，右鍵單擊文件。
2. 選擇 **壓縮**。
3. 輸入壓縮包名稱，選擇壓縮包類型和目標存儲位置。
4. 單擊 **壓縮**。

> ![notes](../common/notes.svg)說明：打開「高級選項」 開關設置解壓密碼和分卷大小。加密文件和分卷壓縮僅支持部分壓縮格式。
...
3. 使用組合鍵  **Ctrl + F** 或在地址欄中單擊搜索按鈕進入搜索狀態，輸入關鍵字後按下鍵盤上的 **Enter** 鍵，搜索相關文件。
...
3. 當窗口有多個標籤頁時，可以：
 - 單擊標籤頁右側的 + 繼續添加標籤頁。
 - 單擊標籤頁上的 × 按鈕或將鼠標置於標籤頁上，單擊鼠標中鍵關閉標籤頁。
...
##### 通過右側訊息欄添加標記
選擇文件(夾)後，單擊 ![fileinfo](../common/fileinfo.svg)，在右側訊息欄中也可以添加標記。
...
1. 在檔案管理員界面上，右鍵單擊文件夾選擇 **共享文件夾**。
2. 在文件夾的屬性窗口，勾選 **共享此文件夾**。
3. 根據需要設置 **共享名**、**權限** 、**匿名訪問** 後關閉窗口。
4. 在檔案管理員界面上，單擊 ![icon_menu](../common/icon_menu.svg) > **設置共享密碼**。
5. 輸入共享密碼，單擊 **確定**。
...
1. 在檔案管理員空白區域，單擊鼠標右鍵。
2. 選擇 **從終端中打開**，會啟動終端應用程序，終端的路徑為當前目錄。
...
2. 選擇 **重命名**。
3. 輸入新名稱後按下鍵盤上的 **Enter** 鍵或單擊空白處保存修改。
...
3. 輸入新的卷標名稱，按下鍵盤上的 **Enter** 鍵或單擊空白處保存修改。
...
2. 選擇 **高級設置** 選項。
   ...
   + 勾選 **使用文件管理器的文件選擇對話框**，選擇文件時打開檔案管理員窗口。
