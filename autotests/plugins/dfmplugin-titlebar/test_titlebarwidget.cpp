...
protected:
    void SetUp() override
    {
        ...
        // Stub windowId to return valid id, preventing Q_ASSERT(id > 0) crashes
...
TEST_F(TitleBarWidgetTest, HandleHotkeyCtrlF_Called_ActivatesSearchEdit)
{
    ...
    // Make widget visible so searchEditWidget->isVisible() returns true
    widget->show();
...
{
    // When only one tab remains, removeTab calls window()->close()
    // and returns early without actually removing the tab.
    // Verify the real DTabBar::removeTab is NOT called.
    bool dtabBarRemoveCalled = false;
    ...
    stub.set_lamda(static_cast<void (DTabBar::*)(int)>(&DTabBar::removeTab),
                   [&dtabBarRemoveCalled](DTabBar *, int) {
        __DBG_STUB_INVOKE__
        dtabBarRemoveCalled = true;
    });

    widget->handleHotketCloseCurrentTab();
    EXPECT_FALSE(dtabBarRemoveCalled);  // window->close() path taken, tab NOT removed
}
