...
{
    // The display preview checkbox now emits displayPreviewVisibleChanged
    // instead of calling DConfigManager::setValue directly
    QSignalSpy spy(widget, &ViewOptionsWidget::displayPreviewVisibleChanged);

    widget->d->displayPreviewCheckBox->setChecked(true);

    ASSERT_EQ(spy.count(), 1);
    EXPECT_TRUE(spy.at(0).at(0).toBool());   // visible = true
    EXPECT_TRUE(spy.at(0).at(1).toBool());   // userAction = true
}
