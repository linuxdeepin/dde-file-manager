...
TEST_F(TabBarTest, RemoveTab_ValidIndex_RemovesTab)
{
    ...
    // When only one tab remains, removeTab now calls window()->close()
    // instead of actually removing the tab, so count stays at 1
...
TEST_F(TabBarTest, RemoveTab_LastTab_CountBecomesZero)
{
    ...
    // instead of removing the tab, so count stays at 1
    EXPECT_EQ(tabBar->count(), 1);
}
