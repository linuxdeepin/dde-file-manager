...
TEST_F(UT_MenuUtils, PerfectMenuParams_PartialKeysMissing_RecalculatesAllKeys)
{
    // 只传 2 个 key（缺少 kIsFocusOnDDEDesktopFile），仍应触发重算分支。
    // 不 stub：/tmp 下的 file:// URL 走真实实现，isComputerDesktopFile 等
    // 与 SystemPathUtil::isSystemPath 均确定性地返回 false。
    QList<QUrl> urls = { QUrl::fromLocalFile("/tmp/menu-utils-a.txt"),
                         QUrl::fromLocalFile("/tmp/menu-utils-b.txt") };

    QVariantHash params;
    params[MenuParamKey::kSelectFiles] = QVariant::fromValue(urls);
    params[MenuParamKey::kIsSystemPathIncluded] = true;   // 故意给错值，重算后应被覆盖为 false
    params[MenuParamKey::kIsDDEDesktopFileIncluded] = false;
