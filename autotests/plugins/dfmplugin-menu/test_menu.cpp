...
public:
    ...
    QString name() const override { return "TestMenuScene"; }
};
...
// extended: init() may be called more than once (plugin re-load), all
// builtin scenes must remain registered afterwards.
TEST_F(UT_MenuHandle, Init_SecondCall_AllScenesStillRegistered)
{
    stub.set_lamda(&MenuHandle::publishSceneAdded, [](MenuHandle *, const QString &) {
        __DBG_STUB_INVOKE__
    });

    ASSERT_TRUE(handle->init());
    ASSERT_TRUE(handle->contains("OemMenu"));

    EXPECT_TRUE(handle->init());

    const QStringList scenes { "NewCreateMenu", "ClipBoardMenu", "OpenDirMenu",
                               "FileOperatorMenu", "OpenWithMenu", "ShareMenu",
                               "ExtendMenu", "OemMenu", "TemplateMenu", "SendToMenu",
                               "DConfigMenuFilter", "ActionIconManager" };
    for (const QString &scene : scenes)
        EXPECT_TRUE(handle->contains(scene)) << qPrintable(scene);
}
...
TEST_F(UT_MenuHandle, PublishSceneAdded_DispatchesEvent)
{
    EXPECT_NO_FATAL_FAILURE(handle->publishSceneAdded("TestScene"));
}

TEST_F(UT_MenuHandle, PublishSceneRemoved_DispatchesEvent)
{
    EXPECT_NO_FATAL_FAILURE(handle->publishSceneRemoved("TestScene"));
}

TEST_F(UT_MenuHandle, CreateSubscene_NullParent_EarlyReturns)
{
    auto creator = new TestSceneCreator();
    creator->addChild("TestScene");   // populate children, parent stays null
    EXPECT_NO_FATAL_FAILURE(handle->createSubscene(creator, nullptr));
    delete creator;
}

TEST_F(UT_MenuHandle, CreateSubscene_NullCreator_EarlyReturns)
{
    TestMenuScene parentScene;
    EXPECT_NO_FATAL_FAILURE(handle->createSubscene(nullptr, &parentScene));
}

TEST_F(UT_MenuHandle, CreateSubscene_ValidCreator_AddsChildScenes)
{
    stub.set_lamda(&MenuHandle::publishSceneAdded, [](MenuHandle *, const QString &) {
        __DBG_STUB_INVOKE__
    });
    stub.set_lamda(VADDR(TestSceneCreator, create), [](AbstractSceneCreator *) -> AbstractMenuScene * {
        __DBG_STUB_INVOKE__
        return new TestMenuScene();
    });

    auto creator = new TestSceneCreator();
    creator->addChild("ChildScene");
    handle->registerScene("TestScene", creator);

    auto childCreator = new TestSceneCreator();   // 无子场景，终止递归
    handle->registerScene("ChildScene", childCreator);

    TestMenuScene parentScene;
    EXPECT_NO_FATAL_FAILURE(handle->createSubscene(creator, &parentScene));
    EXPECT_EQ(parentScene.subscene().size(), 1);
    qDeleteAll(parentScene.subscene());
}
