...
TEST_F(UT_DCustomActionData, IsAction_WithChildren_ReturnsFalse)
{
    data->actionCommand = "test command";
    data->childrenActions.append(DCustomActionData());
    EXPECT_FALSE(data->isAction());
}
...
TEST_F(UT_DCustomActionData, Position_ComboTypeNotConfigured_FallsBackToActionPosition)
{
    data->actionPosition = 7;
    data->comboPos[DCustomActionDefines::ComboType::kSingleFile] = 10;
    EXPECT_EQ(data->position(DCustomActionDefines::ComboType::kBlankSpace), 7);
}
...
TEST_F(UT_DCustomActionData, SelfAssignment_DataIsUnchanged)
{
    data->actionName = "Self";
    data->actionIcon = "self-icon";
    data->actionCommand = "self-command";
    data->actionPosition = 9;
    data->actionParentPath = "/self/path";
    data->comboPos[DCustomActionDefines::ComboType::kBlankSpace] = 3;
    data->childrenActions.append(DCustomActionData());

    DCustomActionData &self = (*data = *data);

    EXPECT_EQ(&self, data);
    EXPECT_EQ(data->name(), "Self");
    EXPECT_EQ(data->icon(), "self-icon");
    EXPECT_EQ(data->command(), "self-command");
    EXPECT_EQ(data->position(), 9);
    EXPECT_EQ(data->parentPath(), "/self/path");
    EXPECT_EQ(data->position(DCustomActionDefines::ComboType::kBlankSpace), 3);
    EXPECT_EQ(data->acitons().size(), 1);
}
...
TEST_F(UT_DCustomActionEntry, CopyConstructor_CopiesAllFields)
{
    ...
    EXPECT_EQ(copy.packageSign, "sign-1234");
}
...
TEST_F(UT_DCustomActionEntry, AssignmentOperator_CopiesAllFields)
{
    ...
    EXPECT_EQ(copy.version(), "1.0");
    EXPECT_EQ(copy.comment(), "Comment");
    EXPECT_EQ(copy.packageSign, "sign-5678");
}

TEST_F(UT_DCustomActionEntry, SelfAssignment_EntryIsUnchanged)
{
    entry->packageName = "SelfPackage";
    entry->packageVersion = "2.0";
    entry->packageSign = "self-sign";
    entry->actionData.actionName = "SelfData";

    DCustomActionEntry &self = (*entry = *entry);

    EXPECT_EQ(&self, entry);
    EXPECT_EQ(entry->package(), "SelfPackage");
    EXPECT_EQ(entry->version(), "2.0");
    EXPECT_EQ(entry->packageSign, "self-sign");
    EXPECT_EQ(entry->data().name(), "SelfData");
}
