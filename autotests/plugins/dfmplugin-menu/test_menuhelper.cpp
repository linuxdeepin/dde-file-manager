...
// 新语义: urls.size() <= kOpenNewWindowMaxCount(50) 直接返回 true；
// 超过 50 时最多扫描前 1000 个，目录数 > 50 返回 false。
...
TEST_F(UT_MenuHelper, CanOpenSelectedItems_AtThreshold_NoScanPerformed)
{
    // 恰好 50 个（kOpenNewWindowMaxCount）时走快速路径，不做任何扫描
    QList<QUrl> urls;
    for (int i = 0; i < 50; ++i)
        urls.append(QUrl::fromLocalFile(QString("/tmp/test%1").arg(i)));
    ...
    stub.set_lamda(&InfoFactory::create<FileInfo>,
                   [&createCallCount](const QUrl &url, Global::CreateFileInfoType, QString *, bool *) -> QSharedPointer<FileInfo> {
                       __DBG_STUB_INVOKE__
                       createCallCount++;
...
TEST_F(UT_MenuHelper, CanOpenSelectedItems_TooManyDirectories_ReturnsFalse)
{
    ...
    EXPECT_FALSE(Helper::canOpenSelectedItems(urls));
}
...
TEST_F(UT_MenuHelper, CanOpenSelectedItems_MixedFilesAndDirs_BelowThreshold_ReturnsTrue)
{
    ...
    stub.set_lamda(VADDR(FileInfo, isAttributes), [&checkCount](FileInfo *, const OptInfoType) {
        __DBG_STUB_INVOKE__
        // 前10个是目录，其余是文件
...
TEST_F(UT_MenuHelper, CanOpenSelectedItems_ScanLimit_StopsScanning)
{
    ...
    stub.set_lamda(VADDR(FileInfo, isAttributes), [](FileInfo *, const OptInfoType) {
        __DBG_STUB_INVOKE__
        return false;   // 都不是目录，扫描不会提前退出
    });

    Helper::canOpenSelectedItems(urls);

    // 应该恰好只扫描前 1000 个
    EXPECT_EQ(createCallCount, 1000);
}
