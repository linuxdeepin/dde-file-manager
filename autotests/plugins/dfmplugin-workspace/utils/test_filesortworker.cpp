...
TEST_F(FileSortWorkerTest, Constructor_ValidUrlAndKey_CreatesWorker)
{
    ...
    // Default sort role is now 0 (unset) until setSortArguments is called
...
TEST_F(FileSortWorkerTest, SetSortArguments_SameArguments_ReturnsOtherChanged)
{
    // Default sort role is 0 (unset), so setting kItemFileDisplayNameRole is a change
...
{
    // Test setting group arguments with unknown strategy name
    // Unknown strategy returns kGroupingOptNone
    Qt::SortOrder order = Qt::DescendingOrder;
...
TEST_F(FileSortWorkerTest, HandleSourceChildren_ValidData_HandlesData)
{
    ...
    // handleSourceChildren was removed, handleAddChildren is now private with different signature
    // worker->handleAddChildren(testKey, children, sortRole, sortOrder, isMixDirAndFile, isFinished);
}
