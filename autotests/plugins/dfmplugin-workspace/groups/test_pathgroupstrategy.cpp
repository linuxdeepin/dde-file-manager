...
{
    // Test getGroupDisplayName for key starting with "other_" (dynamic device key)
    // The group key starts with "other_", so it looks up device info from otherGroupInfos.
    // Since no device info is registered, DeviceUtils::nameOfDefault("", 0) is called.
    QString otherKey = "other_test";
    auto result = strategy->getGroupDisplayName(otherKey);
    
    // Should return a device default name, not the raw key
    EXPECT_FALSE(result.isEmpty());
    EXPECT_NE(result, otherKey);
}
