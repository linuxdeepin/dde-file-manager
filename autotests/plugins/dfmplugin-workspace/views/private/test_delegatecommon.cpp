...
TEST_F(DelegateCommonTest, BoundingPath_SingleRect_ReturnsRoundedRect)
{
    ...
    // boundingPath adds margins (radius+padding, 0, radius+padding, 0) then adjusted(0, -padding, 0, padding)
    // So the actual path rect is (10+7, 20-2, 100+14, 50+4) = (17, 18, 114, 52) with rounded corners
    EXPECT_TRUE(result.contains(testRect));
}
