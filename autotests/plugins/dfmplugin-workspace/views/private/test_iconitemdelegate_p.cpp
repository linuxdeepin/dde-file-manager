...
{
    // Test initial value of checkedIcon - may be null in offscreen/test environments
    // where icon themes are unavailable
    // The actual initialization depends on theme availability
    // Just verify no crash when accessing
    EXPECT_NO_THROW({ (void)d->checkedIcon.isNull(); });
}
...
{
    // Test setting checkedIcon - in offscreen env, theme icons may be null
