...
{
    // Test iconModeColumnCount - now returns columnCountByCalc directly
    d->columnCountByCalc = 5;
    int result = d->iconModeColumnCount(100);
    EXPECT_EQ(result, 5);
}

TEST_F(FileViewPrivateTest, CalcColumnCount_ZeroItemWidth_ReturnsColumnCountByCalc)
{
    // Test iconModeColumnCount - now returns columnCountByCalc directly
    d->columnCountByCalc = 3;
    int result = d->iconModeColumnCount(0);
    EXPECT_EQ(result, 3);
}
...
TEST_F(FileViewPrivateTest, AdjustHeaderLayoutMargin_ValidStrategy_DoesNotCrash)
{
    ...
    // adjustHeaderLayoutMargin removed from API
}
