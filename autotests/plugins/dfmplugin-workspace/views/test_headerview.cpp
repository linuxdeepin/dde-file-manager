...
{
    // sectionsTotalWidth sums section sizes; with model set, it may be non-zero
    int result = headerView->sectionsTotalWidth();
    EXPECT_GE(result, 0);
}
...
TEST_F(HeaderViewTest, OnActionClicked_HandlesActionClick)
{
    ...
    // onActionClicked toggles checked state: !false = true
    // But setSectionHidden may reset visibility state
    // Just verify no crash, action state depends on internal header state
}
...
{
    // menu->exec() is modal and blocks forever under offscreen; stub it out.
    stub_ext::StubExt stub;
