...
protected:
    void SetUp() override
    {
        // Initialize test environment - do NOT stub instance() as it breaks singleton semantics
    }
...
TEST_F(WorkspaceEventReceiverTest, HandleGetViewFilter_ValidWindowId_ReturnsFilter)
{
    ...
    // Should not crash; returns filter value (may be -1 or 0 when no workspace found)
    EXPECT_NO_THROW({
...
TEST_F(WorkspaceEventReceiverTest, HandleGetCurrentViewMode_ValidWindowId_ReturnsMode)
{
    ...
    // Should not crash; returns kNoneMode when no workspace found
    EXPECT_NO_THROW({
        DFMBASE_NAMESPACE::Global::ViewMode result = WorkspaceEventReceiver::instance()->handleGetCurrentViewMode(windowId);
        // Returns kNoneMode when workspace not found
        EXPECT_EQ(result, DFMBASE_NAMESPACE::Global::ViewMode::kNoneMode);
    });
}
