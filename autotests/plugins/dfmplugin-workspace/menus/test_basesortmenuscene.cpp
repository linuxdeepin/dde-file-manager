...
TEST_F(BaseSortMenuSceneTest, Create_NullParent_ReturnsTrue)
{
    // Test scene creation with null parent menu
    // AbstractMenuScene::create always returns true (delegates to sub-scenes)
...
TEST_F(BaseSortMenuSceneTest, Triggered_ValidAction_ReturnsBaseResult)
{
    ...
    // AbstractMenuScene::triggered returns false when no sub-scenes handle it
    EXPECT_FALSE(result);
}
