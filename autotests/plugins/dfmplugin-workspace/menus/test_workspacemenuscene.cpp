...
{
    // Test scene initialization with valid parameters including currentDir
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
    
    bool result = scene->initialize(params);
    
    // initialize now requires a valid currentDir; sub-scene creation may fail
    // in test env but initialize itself should succeed when currentDir is set.
    // However, menuSceneCreateScene may return nullptr for unregistered scenes,
    // so the result depends on AbstractMenuScene::initialize(params).
    // Just ensure no crash and check the return.
    EXPECT_NO_FATAL_FAILURE(result = scene->initialize(params));
}

TEST_F(WorkspaceMenuSceneTest, Initialize_EmptyParams_ReturnsFalse)
{
    // Test scene initialization with empty parameters
    // initialize now returns false when currentDir is empty
    QVariantHash params;
...
TEST_F(WorkspaceMenuSceneTest, Triggered_ValidAction_ReturnsFalse)
{
    // Test triggering valid action without initialization
    // isEmptyArea defaults to false, so normalMenuTriggered is called.
    // The action has no property, so scene() returns nullptr, causing false.
    QAction action("Test Action");
...
TEST_F(WorkspaceMenuSceneTriggerTest, EmptyMenuTriggered_ValidAction_ReturnsFalse)
{
    // emptyMenuTriggered is private and cannot be invoked via QMetaObject::invokeMethod.
    // The actual triggered() dispatches to emptyMenuTriggered internally.
    // Test that triggered() with isEmptyArea=true and no matching action returns false.
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
...
TEST_F(WorkspaceMenuSceneTriggerTest, NormalMenuTriggered_ValidAction_ReturnsFalse)
{
    // normalMenuTriggered is private and cannot be invoked via QMetaObject::invokeMethod.
    // Test that triggered() with isEmptyArea=false and no matching action returns false.
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
...
{
    // Test triggered with empty area parameters - needs valid currentDir
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
    params[MenuParamKey::kIsEmptyArea] = true;
    ...
    // No matching sub-scene, returns false
    EXPECT_FALSE(result);
}

TEST_F(WorkspaceMenuSceneTriggerTest, Triggered_WithNonEmptyAreaParams_HandlesCorrectly)
{
    // Test triggered with non-empty area parameters - needs valid currentDir
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
    params[MenuParamKey::kIsEmptyArea] = false;
...
{
    // Test triggered with window ID parameters - needs valid currentDir
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
...
{
    // Test triggered with select files parameters
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
...
{
    // Test triggered with custom action ID - needs valid currentDir
    QVariantHash params;
    params[MenuParamKey::kCurrentDir] = QUrl("file:///test");
    params[MenuParamKey::kWindowId] = quint64(12345);
    ...
    // Custom action ID doesn't belong to any sub-scene, returns false
    EXPECT_FALSE(result);
}
