...
TEST_F(UT_SizeSlider, Constructor_CreatesSlider)
{
    ...
    // DSlider and QLabel are created in init(), not in constructor
    EXPECT_EQ(slider->findChild<DTK_WIDGET_NAMESPACE::DSlider*>(), nullptr);
    EXPECT_EQ(slider->findChild<QLabel*>(), nullptr);
}
...
TEST_F(UT_SizeSlider, Inheritance_FromContentBackgroundWidget)
{
    ...
    EXPECT_EQ(testSlider.radius(), 8);  // Inherits default radius 8
...
{
    // DSlider and QLabel are created in init(), call init first
    slider->init();
