...
TEST_F(UT_CollectionTitleBar, TestConstructor)
{
    ...
    // Widget is not shown by default, so titleBarVisible() returns false
    ...
    // titleName is empty by default
    EXPECT_TRUE(titleBar->titleName().isEmpty());
}
...
TEST_F(UT_CollectionTitleBar, TestTitleBarVisible)
{
    ...
    // Test setting visibility to true (show widget)
    titleBar->show();
    visible = titleBar->titleBarVisible();
    EXPECT_TRUE(visible);

    // Test after setting to false (hide widget)
    titleBar->hide();
    visible = titleBar->titleBarVisible();
    EXPECT_FALSE(visible);
