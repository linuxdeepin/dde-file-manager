...
TEST_F(UT_Surface, MapToGridSize_ConvertsPixelSizeToGrid)
{
    ...
    // Implementation: w/cw + 1
    EXPECT_EQ(gridSize.width(), 3);   // 40/20 + 1 = 3
    EXPECT_EQ(gridSize.height(), 2);  // 30/20 + 1 = 2
}
...
TEST_F(UT_Surface, PointsDistance_CalculatesDistance)
{
    ...
    // Implementation uses manhattanLength: |3| + |4| = 7
...
{
    Surface surface(nullptr);
    QRect gridRect(1, 1, 2, 3);
    QRect pixelRect = surface.mapToPixelSize(gridRect);

    // Size calculation: w * cellWidth = 2*20 = 40, h * cellWidth = 3*20 = 60
    EXPECT_EQ(pixelRect.width(), 40);
    EXPECT_EQ(pixelRect.height(), 60);
    // Position depends on gridOffset() which varies with widget size
    // Just verify the method doesn't crash
    EXPECT_TRUE(pixelRect.isValid());
}

TEST_F(UT_Surface, MapToGridGeo_ConvertsPixelToGrid)
{
    Surface surface(nullptr);
    QRect pixelRect(25, 35, 45, 55);
    QRect gridRect = surface.mapToGridGeo(pixelRect);

    // Size: w/cw + 1 = 45/20 + 1 = 3, h/cw + 1 = 55/20 + 1 = 3
    EXPECT_EQ(gridRect.width(), 3);
    EXPECT_EQ(gridRect.height(), 3);
    // Position depends on gridOffset() which varies with widget size
    // Just verify the method doesn't crash
    EXPECT_TRUE(gridRect.isValid());
}
