...
TEST_F(UT_CollectionDataProvider, KeyTest)
{
    ...
    // key() looks up URL in collections; no collections means empty keys
    QString key1 = provider->key(url1);
    QString key2 = provider->key(url2);
    QString key3 = provider->key(url3);

    EXPECT_TRUE(key1.isEmpty());
    EXPECT_TRUE(key2.isEmpty());
...
TEST_F(UT_CollectionDataProvider, KeysTest)
{
    ...
    // MockCollectionDataProvider::append returns empty string,
    // so no collections are actually added
    provider->append(QUrl("file:///test1.txt"));
    QStringList keys2 = provider->keys();
    EXPECT_TRUE(keys2.isEmpty());
    ...
    EXPECT_TRUE(keys3.isEmpty());  // Mock doesn't add collections
}
