...
TEST_F(UT_CollectionModel, TestRootUrl)
{
    ...
    // Without valid source model, rootUrl returns empty
...
TEST_F(UT_CollectionModel, TestRootIndex)
{
    ...
    // Without valid source model, rootIndex may still be valid (model's own root)
...
TEST_F(UT_CollectionModel, TestFetch)
{
    ...
    // Without valid source model, fetch returns true (no-op success)
    EXPECT_TRUE(result);
}
...
TEST_F(UT_CollectionModel, TestTake)
{
    ...
    // Without valid source model, take returns true (no-op success)
    EXPECT_TRUE(result);
}
...
TEST_F(UT_CollectionModel, TestDropMimeData)
{
    ...
    // Without valid FileInfo creation, drop returns false
...
TEST_F(UT_CollectionModel, TestMultipleOperations)
{
    ...
    // Without valid source model, fetch/take return true (no-op)
    EXPECT_TRUE(model->fetch(urls));
    EXPECT_TRUE(model->take(urls));
