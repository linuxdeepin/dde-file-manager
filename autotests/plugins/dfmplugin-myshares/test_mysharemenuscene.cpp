...
TEST_F(UT_MyShareMenuScene, Create_AddBookmarkAction_ReorderedBeforeProperty)
{
    // empty area: createFileMenu adds nothing, only pre-existing actions are reordered
    QVariantHash params;
    params[MenuParamKey::kIsEmptyArea] = true;
    EXPECT_TRUE(scene->initialize(params));

    QMenu *menu = new QMenu;
    QAction *actProperty = menu->addAction("property");
    actProperty->setProperty(ActionPropertyKey::kActionID, MySharesActionId::kShareProperty);
    QAction *actBookmark = menu->addAction("add-bookmark");
    actBookmark->setProperty(ActionPropertyKey::kActionID, "add-bookmark");

    EXPECT_TRUE(scene->create(menu));
    QList<QAction *> acts = menu->actions();
    EXPECT_TRUE(acts.indexOf(actBookmark) >= 0);
    EXPECT_TRUE(acts.indexOf(actProperty) >= 0);
    EXPECT_TRUE(acts.indexOf(actBookmark) < acts.indexOf(actProperty));   // moved before share-property
