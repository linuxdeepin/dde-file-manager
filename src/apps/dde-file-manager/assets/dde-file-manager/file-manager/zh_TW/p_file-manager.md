...
1. 在電腦介面，右鍵單擊需要加密的磁碟，選擇 **開啟分區加密**。

   ==（图片需更换）==

2. 在跳出視窗中設定解鎖方式後，點擊 **下一步**。推薦選擇“在此電腦上使用TPM+PIN碼解鎖（推薦）”。

3. 選擇復原金鑰文件匯出路徑後，點擊 **下一步**。只能選擇非加密分區匯出復原金鑰，可以匯出到本機磁碟或隨身碟等外設。

4. 點擊 **確認加密** 後，輸入使用者密碼，重啟電腦進入初始化介面進行加密配置。

==（图片需更换）==

5. 在系統初始化介面中，依次輸入密碼、重複密碼，輸入"y"確認加密。
...
> ![notes](../common/notes.svg) 說明：
> 
> 請務必妥善保管復原金鑰，在忘記密碼或加密硬體損壞等特殊場景下，可以使用復原金鑰解鎖加密分區。
> 
> 加密過程可能造成系統卡頓，請盡量減少系統操作。

#### ==解密分區==

1. ==使用“在此電腦上使用TPM+PIN碼解鎖（推薦）”解鎖方式時，在系統初始化介面輸入分區密碼進行解鎖。==
2. ==使用“在此電腦上使用TPM自動解鎖”解鎖方式時，系統將自動校驗解鎖。==
3. ==使用“透過口令解鎖”解鎖方式時，在電腦介面打開分區時輸入口令密碼解鎖。==

#### ==修改密碼==

1. 在電腦介面，右鍵單擊已加密的磁碟，選擇 **修改加密口令** 或 **修改加密PIN碼。

==（图片需更换）==

2. 可以點擊切換 **使用密碼驗證** 和 **使用復原金鑰驗證**。

3. 輸入舊密碼或復原金鑰，輸入、重複輸入新密碼，點擊 **修改** ，輸入使用者密碼驗證後完成密碼修改。

==（图片需更换）==

#### ==取消分區加密==

1. 在電腦介面，右鍵單擊已加密的磁碟，選擇 **取消分區加密**。

2. 輸入使用者密碼驗證後，在取消加密跳出視窗中選擇 **立即重啟** 或 **稍後重啟** ，重啟電腦進入初始化介面。

3. 在系統初始化介面中，依次輸入分區密碼，輸入"y"確認取消加密。 
