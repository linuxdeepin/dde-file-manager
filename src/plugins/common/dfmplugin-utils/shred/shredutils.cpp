...
bool ShredUtils::isValidFile(const QUrl &file)
{
    ...
    if (realPath.startsWith("/media/")) {
        QStorageInfo storage(realPath);
        // 检查设备是否是可移动设备
...
int ShredUtils::countFilesInDirectory(const QString &dirPath)
{
    // This method is kept for compatibility but logic moved to FileShredWorker
    // For now, return 0 as this should not be called in the new architecture
    Q_UNUSED(dirPath);
    fmWarning() << "ShredUtils::countFilesInDirectory should not be called in new architecture";
    return 0;
}
