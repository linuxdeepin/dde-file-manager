...
{
    QString errString;
    auto fileInfo = DFMBASE_NAMESPACE::InfoFactory::create<FileInfo>(url, true, &errString);
...
bool OemMenuPrivate::isAllEx7zFile(const QList<QUrl> &files) const
{
    ...
    for (const QUrl &f : files) {
        ...
        auto fileInfo = DFMBASE_NAMESPACE::InfoFactory::create<FileInfo>(f, true, &errString);
        if (fileInfo.isNull()) {
            qDebug() << errString;
            return false;
        }
...
QList<QAction *> OemMenu::normalActions(const QList<QUrl> &files, bool onDesktop)
{
    ...
    QString errString;
    if (1 == files.count()) {
        auto fileInfo = DFMBASE_NAMESPACE::InfoFactory::create<FileInfo>(files.first(), true, &errString);
        if (!fileInfo) {
            qDebug() << errString;
            return {};
        }
    ...
    for (const QUrl &file : files) {
        ...
        auto fileInfo = DFMBASE_NAMESPACE::InfoFactory::create<FileInfo>(file, true, &errString);
