...
void TemplateMenuPrivate::createActionByNormalFile(const QString &path)
{
    ...
    if (!fileInfo) {
        qInfo() << "createActionByDesktopFile create SyncFileInfo error: " << errString << path;
        return;
    }
...
void TemplateMenuPrivate::createActionByDesktopFile(const QDir &dir, const QString &path)
{
    ...
    if (Q_UNLIKELY(!itemInfo)) {
        qInfo() << "createActionByDesktopFile create SyncFileInfo error: " << errString << entrySourcePath;
        return;
    }
