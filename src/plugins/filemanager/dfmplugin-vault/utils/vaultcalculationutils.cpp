...
VaultCalculationUtils::VaultCalculationUtils(QObject *parent)
    : QObject(parent)
{
    thread = new QThread();
    connect(this, &VaultCalculationUtils::sigStartRun, this, &VaultCalculationUtils::statisticsFileInfo, Qt::BlockingQueuedConnection);
    moveToThread(thread);
    thread->start();
}

VaultCalculationUtils::~VaultCalculationUtils()
{
    thread->wait();
    thread->terminate();
    if (info) {
...
void VaultCalculationUtils::startThread(const QList<QUrl> &files)
{
    emit sigStartRun(files);
}

void VaultCalculationUtils::statisticsFileInfo(const QList<QUrl> &files)
{
    if (files.isEmpty())
        return;
...
quint16 VaultCalculationUtils::getMemoryPageSize()
{
...
void VaultCalculationUtils::statisticFilesSize(const QUrl &url, FileStatisticInfo *&sizeInfo)
{
