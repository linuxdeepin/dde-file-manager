...
void SearchHelper::authorizeSearchExperience()
{
    ...
    // 点击"查看索引状态"按钮时，通过 kShowSettingDialogWithGroup 信号
    // 打开设置窗口并自动跳转到"高级设置—搜索"分组。
    ...
    UniversalUtils::notifyMessage(
            ...
            [](const QString &actionId) {
                if (actionId == "view-index-status") {
                    auto window = FMWindowsIns.findWindowById(FMWindowsIns.lastActivedWindowId());
                    if (window) {
                        dpfSignalDispatcher->publish(
                                DFMBASE_NAMESPACE::GlobalEventType::kShowSettingDialogWithGroup,
                                window->winId(),
                                QString(SEARCH_SETTING_GROUP));
                    }
                }
            });
}
