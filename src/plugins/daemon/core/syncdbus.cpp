...
SyncDBus::SyncDBus(QObject *parent)
    : QObject(parent), m_threadPool(new QThreadPool(this)), m_nextTaskId(1)
{
...
int SyncDBus::SyncFS(const QString &path, const QVariantMap &options)
{
    ...
    // Create sync task; manual lifecycle management required because
    // taskCompleted uses QueuedConnection — QThreadPool must NOT delete
    // the task before the queued slot finishes reading its members.
...
void SyncDBus::onSyncTaskCompleted(SyncTask *task)
{
    ...
    // Manually delete the task (autoDelete is disabled — see SyncFS)
    task->deleteLater();
}
...
SyncTask::SyncTask(int taskId, const QString &path, const QVariantMap &options, QObject *parent)
    : QObject(parent), m_taskId(taskId), m_path(path), m_options(options), m_success(false)
{
}
