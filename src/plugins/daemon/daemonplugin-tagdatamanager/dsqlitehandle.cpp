                ...
                has_member_function(cbegin)
                        has_member_function(cend)
                                has_member_function(push_back)
                                        ...
                                        template<typename __Ty, typename __TR, typename = typename std::enable_if < std::is_same<__Ty, __Ty &>::value && std::is_same<typename std::remove_reference<typename std::remove_cv<__Ty>::type>::type, QMap<QString, QList<QString>>>::value>
                                        ::type > __Ty &operator+=(__Ty &&lh, const __TR &rh) noexcept
{
    if (rh.isEmpty()) {
        return lh;
    }
...
template<typename __Ty, typename __TR, typename std::enable_if<std::is_same<__Ty, __Ty &>::value && has_member_cbegin<typename std::remove_reference<typename std::remove_cv<__Ty>::type>::type>::value && has_member_cend<typename std::remove_reference<typename std::remove_cv<__Ty>::type>::type>::value && has_member_push_back<typename std::remove_reference<typename std::remove_cv<__Ty>::type>::type>::value>::type>
...
DSqliteHandle::DSqliteHandle(QObject *const parent)
    : QObject(parent),
      sqlDatabasePtr(new QSqlDatabase())
{
    std::lock_guard<std::mutex> raiiLock { mutex };
    ...
    if (!partionsAndMounPoints.empty()) {
        partionsOfDevices.reset(new std::map<QString, std::multimap<QString, QString>> { partionsAndMounPoints });
    }
...
QPair<QString, QString> DSqliteHandle::getMountPointOfFile(QUrl url,
                                                           std::unique_ptr<std::map<QString, std::multimap<QString, QString>>> &partionsAndMountPoints)
{
    QFileInfo info(url.path(QUrl::FullyDecoded));
    while (!info.exists()) {
        const QUrl &parentUrl = DAEMONPTAGDM_NAMESPACE::parentUrl(url);

        if (!parentUrl.isValid() || parentUrl == url) {
            break;
        }

        url = parentUrl;
        info = QFileInfo(url.path(QUrl::FullyDecoded));
    }

    QPair<QString, QString> partionAndMountPoint {};

    if (info.exists() && partionsAndMountPoints
        && !partionsAndMountPoints->empty()) {
        QString path = DAEMONPTAGDM_NAMESPACE::parentUrl(url).path(QUrl::FullyDecoded);
...
void DSqliteHandle::onMountAdded(const QString &deviceId, const QString &mountPoint)
{
    (void)deviceId;
    ...
    partionsOfDevices.reset(nullptr);

    if (!partionsAndMountPoints.empty()) {
        partionsOfDevices = std::unique_ptr<std::map<QString, std::multimap<QString, QString>>> {
...
void DSqliteHandle::onMountRemoved(const QString &deviceId)
{
    (void)deviceId;
...
QVariant DSqliteHandle::disposeClientData(const QMap<QString, QList<QString>> &filesAndTags, const unsigned long long &type)
{
    ...
    if (!filesAndTags.isEmpty() && type != 0) {
        ...
        case 9: {
            std::lock_guard<std::mutex> raillLock { mutex };
        ...
        case 11: {
            std::lock_guard<std::mutex> raii_lock { mutex };
...
void DSqliteHandle::connectToSqlite(const QString &path, const QString &db_name)
{
    ...
    std::function<void()> initDatabasePtr { [&] {
        if (sqlDatabasePtr->isOpen()) {
            sqlDatabasePtr->close();
        }

        if (QSqlDatabase::contains(CONNECTIONNAME)) {
            sqlDatabasePtr.reset(nullptr);
            QSqlDatabase::removeDatabase(CONNECTIONNAME);
        }

        sqlDatabasePtr = std::unique_ptr<QSqlDatabase> { new QSqlDatabase { QSqlDatabase::addDatabase(R"foo(QSQLITE)foo", CONNECTIONNAME) } };
        QString DBName { path + QString { "/" } + db_name };

        ///###: for debugging.
        //            qDebug() << DBName;

        sqlDatabasePtr->setDatabaseName(DBName);
        sqlDatabasePtr->setUserName(USERNAME);
        sqlDatabasePtr->setPassword(PASSWORD);
    } };

    if (code == DSqliteHandle::ReturnCode::NoExist) {
        initDatabasePtr();

        if (sqlDatabasePtr->open()) {
            if (sqlDatabasePtr->transaction()) {
                QSqlQuery sqlQuery { *sqlDatabasePtr };
                ...
                if (!sqlDatabasePtr->commit()) {
                    sqlDatabasePtr->rollback();
                }
...
{
    QString dataBaseFullPath(kDataBasePath);
    dataBaseFullPath = dataBaseFullPath + "/" + kDataBaseName;
    //检查share目录中是否已有数据库存在
    DSqliteHandle::ReturnCode code = this->checkDBFileExist(dataBaseFullPath, db_name);

    //如果目录不存在则创建该目录
    if (code == DSqliteHandle::ReturnCode::NoThisDir) {
        QDir dir(kDataBasePath);
        int mkdRet = dir.mkdir(kDataBaseName);
        if (mkdRet) {
            code = this->checkDBFileExist(dataBaseFullPath, db_name);
        } else {
            qDebug() << "errors occured when make user local folder";
        }
    }

    connectToSqlite(dataBaseFullPath, db_name);
    if (code == DSqliteHandle::ReturnCode::NoExist) {
        ...
        QString shareDBPath = dataBaseFullPath;
...
template<>
bool DSqliteHandle::helpExecSql<DSqliteHandle::SqlType::TagFiles, QMap<QString, QList<QString>>, bool>(const QMap<QString, QList<QString>> &forDecreasing, const QString &mountPoint)
{
    if (!forDecreasing.isEmpty() && !mountPoint.isEmpty()) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                itrOfSqlForDeleting { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::TagFiles) };
...
bool DSqliteHandle::helpExecSql<DSqliteHandle::SqlType::TagFilesThroughColor,
                                ...
                                bool>(const std::list<std::tuple<QString, QString, QString, QString, QString, QString>> &sqlStrs,
                                      const QString &mountPoint)
{
    if (!flag.load(std::memory_order_consume) && !sqlStrs.empty() && !mountPoint.isEmpty()) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::TagFilesThroughColor2) };
...
bool DSqliteHandle::helpExecSql<DSqliteHandle::SqlType::TagFilesThroughColor3, QString, bool>(const QString &tag_name, const QString &mountPoint)
{
    if (!tag_name.isEmpty() && mountPoint == QString { "/home" }) {
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::TagFilesThroughColor3) };
        QSqlQuery sql_query { *sqlDatabasePtr };
...
bool DSqliteHandle::helpExecSql<DSqliteHandle::SqlType::UntagSamePartionFiles2, QMap<QString, QList<QString>>>(const QMap<QString, QList<QString>> &fileNameAndTagNames,
                                                                                                               const QString &mountPoint)
{
    if (!fileNameAndTagNames.isEmpty() && !mountPoint.isEmpty() && static_cast<bool>(sqlDatabasePtr)) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::UntagSamePartionFiles2) };
        QSqlQuery sqlQuery { *sqlDatabasePtr };
                                ...
                                std::list<QString>, bool>(const std::list<QString> &files, const QString &mount_point)
{
    ...
    if (!files.empty() && !mount_point.isEmpty()) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::DeleteFiles) };
        QSqlQuery sql_query { *sqlDatabasePtr };
                                                         ...
                                                         std::list<QString>, QMap<QString, QList<QString>>>(const std::list<QString> &files, const QString &mount_point)
{
    ...
    if (!files.empty() && !mount_point.isEmpty()) {
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::DeleteFiles2) };
        QSqlQuery sql_query { *sqlDatabasePtr };
                                ...
                                QMap<QString, QList<QString>>>(const QMap<QString, QList<QString>> &fileNameAndTagNames, const QString &mountPoint)
{
    ...
    if (!fileNameAndTagNames.isEmpty() && !mountPoint.isEmpty()) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::DeleteTags2) };
        ...
        if (!filesNameAndTagsName.empty()) {
            ...
            for (; itrForUpdatingOrDeleting != itrForUpdatingOrDeletingEnd; ++itrForUpdatingOrDeleting) {
                ...
                if (!flag.load(std::memory_order_consume) && !sqlForUpdatingOrDeleting.isEmpty()) {

                    if (!sqlQuery.exec(sqlForUpdatingOrDeleting)) {
                        qWarning() << sqlQuery.lastError().text();
                    }

                } else if (flag.load(std::memory_order_consume) && !sqlForUpdatingOrDeleting.isEmpty()) {
                    DSqliteHandle::ReturnCode code { this->checkDBFileExist(mountPoint) };
...
bool DSqliteHandle::helpExecSql<DSqliteHandle::SqlType::DeleteTags3, QList<QString>, bool>(const QList<QString> &tag_name, const QString &mountPoint)
{
    if (!tag_name.isEmpty() && mountPoint == QString { "/home" } && QFileInfo::exists("/home")) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::DeleteTags3) };
                                                         ...
                                                         QMap<QString, QList<QString>>>(const std::map<QString, QString> &files, const QString &mount_point)
{
    ...
    if (!files.empty()) {
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(SqlType::ChangeFilesName2) };
                                                                                                              ...
                                                                                                              const QString &mountPoint)
{
    if (!old_and_new.empty() && mountPoint == QString { "/home" } && QFileInfo::exists("/home")) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(SqlType::ChangeTagsName2) };
        QSqlQuery sql_query { *sqlDatabasePtr };
                                                         ...
                                                         QList<QString>, QMap<QString, QList<QString>>>(const QList<QString> &tag_names, const QString &mount_point)
{
    ...
    if (!tag_names.isEmpty()) {
        ...
        if (partionsOfDevices && !partionsOfDevices->empty()) {
            std::map<QString, std::multimap<QString, QString>>::const_iterator device_itr_beg { partionsOfDevices->cbegin() };
            std::map<QString, std::multimap<QString, QString>>::const_iterator device_itr_end { partionsOfDevices->cend() };
            std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                      std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                    range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::DeleteTags4) };
            ...
            for (; device_itr_beg != device_itr_end; ++device_itr_beg) {
                ...
                for (; partion_itr_beg != partion_itr_end; ++partion_itr_beg) {
                    ...
                    if (code == DSqliteHandle::ReturnCode::NoExist || code == DSqliteHandle::ReturnCode::Exist) {
                        ...
                        if (sqlDatabasePtr && sqlDatabasePtr->open()) {
                            QSqlQuery sql_query { *sqlDatabasePtr };
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::TagFiles, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    if (!filesAndTags.isEmpty()) {
        ...
        QPair<QString, QString> unixDeviceAndMountPoint { DSqliteHandle::getMountPointOfFile(
                QUrl::fromLocalFile(filesAndTags.cbegin().key()), partionsOfDevices) };
        ...
        if (!mutualTags.isEmpty()) {
            ...
            if (code == DSqliteHandle::ReturnCode::Exist || code == DSqliteHandle::ReturnCode::NoExist) {
                ...
                if (sqlDatabasePtr && sqlDatabasePtr->open() && sqlDatabasePtr->transaction()) {
                    ...
                    if (!(valueOfDelRedundant && valueOfInsertNew && valueOfUpdating && sqlDatabasePtr->commit())) {
                        sqlDatabasePtr->rollback();
                        this->closeSqlDatabase();
            ...
            if (code == DSqliteHandle::ReturnCode::Exist || code == DSqliteHandle::ReturnCode::NoExist) {
                ...
                if (sqlDatabasePtr && sqlDatabasePtr->open() && sqlDatabasePtr->transaction()) {
                    ...
                    if (!(valueOfInsertNew && valueOfUpdating && sqlDatabasePtr->commit())) {
                        sqlDatabasePtr->rollback();
                        this->closeSqlDatabase();
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::TagFilesThroughColor, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    ...
    if (!filesAndTags.isEmpty()) {
        ...
        if (!(the_result && sqlDatabasePtr->commit())) {
            sqlDatabasePtr->rollback();
            this->closeSqlDatabase();
        ...
        for (; cbeg != cend; ++cbeg) {
            QPair<QString, QString> unixDeviceAndMountPoint { DSqliteHandle::getMountPointOfFile(QUrl::fromLocalFile(cbeg.key()), partionsOfDevices) };
        ...
        for (; cbeg != cend; ++cbeg) {
            std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                      std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                    range { SqlTypeWithStrs.equal_range(SqlType::TagFilesThroughColor) };
            ...
            if (code == DSqliteHandle::ReturnCode::Exist || code == DSqliteHandle::ReturnCode::NoExist) {
                ...
                if (static_cast<bool>(sqlDatabasePtr)) {
                    ...
                    if (!sqlStrs.empty()) {
                        if (sqlDatabasePtr->open() && sqlDatabasePtr->transaction()) {
        ...
        if (code == DSqliteHandle::ReturnCode::Exist || code == DSqliteHandle::ReturnCode::NoExist) {
            ...
            if (static_cast<bool>(sqlDatabasePtr)) {
                std::pair<std::multimap<SqlType, QString>::const_iterator,
                          std::multimap<SqlType, QString>::const_iterator>
                        range { SqlTypeWithStrs.equal_range(SqlType::UntagSamePartionFiles) };
                ...
                if (range.first != range.second) {
                    ...
                    if (!sqlForDeletingRowOfTagWithFile.empty() && sqlDatabasePtr->open()
                        && sqlDatabasePtr->transaction()) {
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::DeleteFiles, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    if (!filesAndTags.isEmpty()) {
        ...
        std::pair<std::map<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::map<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::DeleteFiles) };
        ...
        for (; cbeg != cend; ++cbeg) {
            QPair<QString, QString> unixDeviceAndMountPoint {
                DSqliteHandle::getMountPointOfFile(QUrl::fromLocalFile(cbeg.key()), partionsOfDevices)
            };
        ...
        for (; itr_partion_and_files != itr_partion_and_files_end; ++itr_partion_and_files) {
            ...
            if (code == DSqliteHandle::ReturnCode::NoExist || code == DSqliteHandle::ReturnCode::Exist) {
                ...
                if (sqlDatabasePtr && sqlDatabasePtr->open() && sqlDatabasePtr->transaction()) {
                    ...
                    if (!(result && sqlDatabasePtr->commit())) {
                        sqlDatabasePtr->rollback();
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::DeleteTags, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    if (!filesAndTags.isEmpty()) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::DeleteTags) };
        ...
        if (partionsOfDevices && !partionsOfDevices->empty()) {
            std::map<QString, std::multimap<QString, QString>>::const_iterator deviceItr { partionsOfDevices->cbegin() };
            std::map<QString, std::multimap<QString, QString>>::const_iterator deviceItrEnd { partionsOfDevices->cend() };
                ...
                for (; mountPointItr != mountPointItrEnd; ++mountPointItr) {
                    ...
                    if (!mountPointItr->second.isEmpty() && !mountPointItr->second.isNull()) {
                        ...
                        if (code == DSqliteHandle::ReturnCode::NoExist || code == DSqliteHandle::ReturnCode::Exist) {
                            ...
                            if (!(flagForDeleteInTagWithFile && flagForUpdatingFileProperty && sqlDatabasePtr && sqlDatabasePtr->commit())) {
                                sqlDatabasePtr->rollback();
                                result = false;
                            }
                        }
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::ChangeFilesName, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    if (!filesAndTags.isEmpty()) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::ChangeFilesName) };
                ...
                for (const std::pair<QString, std::map<QString, QString>> &mountPointAndSqls : sqlForChangingFilesName) {
                    ...
                    if (code == DSqliteHandle::ReturnCode::NoExist || code == DSqliteHandle::ReturnCode::Exist) {
                        ...
                        if (sqlDatabasePtr && sqlDatabasePtr->open() && sqlDatabasePtr->transaction()) {
                            ...
                            if (!(resultOfExecSql && sqlDatabasePtr->commit())) {
                                sqlDatabasePtr->rollback();
                                result = false;
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::ChangeTagsName, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    if (!filesAndTags.isEmpty()) {
        ...
        std::pair<std::map<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::map<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::ChangeTagsName) };
                ...
                for (; mountPointItr != mountPointItrEnd; ++mountPointItr) {
                    ...
                    if (!mountPointItr->second.isEmpty() && !mountPointItr->second.isNull()) {
                        ...
                        if (code == DSqliteHandle::ReturnCode::NoExist || code == DSqliteHandle::ReturnCode::Exist) {
                            ...
                            if (sqlDatabasePtr && sqlDatabasePtr->open()) {
                                flagOfTransaction = sqlDatabasePtr->transaction();
                                ...
                                if (flagOfTransaction) {
                                    resultOfChangeNameOfTag = this->helpExecSql<DSqliteHandle::SqlType::ChangeTagsName,
                                                                                std::list<std::tuple<QString, QString,
                                                                                                     QString, QString>>,
                                                                                bool>(sqlStrs, mountPointItr->second);
                                }
                            }

                            if (!(resultOfChangeNameOfTag && flagOfTransaction && sqlDatabasePtr && sqlDatabasePtr->commit())) {
                                sqlDatabasePtr->rollback();
                                result = false;
                            }
                        }
...
QList<QString> DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::GetTagsThroughFile, QList<QString>>(const QMap<QString, QList<QString>> &filesAndTags)
{
    ...
    if (!filesAndTags.isEmpty()) {
        QMap<QString, QList<QString>>::const_iterator cbeg { filesAndTags.cbegin() };
        QPair<QString, QString> partionAndMountPoint { DSqliteHandle::getMountPointOfFile(QUrl::fromLocalFile(cbeg.key()), partionsOfDevices) };
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::GetTagsThroughFile) };
    ...
    std::transform(tags.begin(), tags.end(), std::back_inserter(tags_backup),
                   [](const QString &tag) {
                       return DAEMONPTAGDM_NAMESPACE::restoreEscapedEnSkim(tag);
                   });
...
QList<QString> DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::GetFilesThroughTag, QList<QString>>(const QMap<QString, QList<QString>> &filesAndTags)
{
    ...
    if (!filesAndTags.isEmpty()) {
        ...
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::GetFilesThroughTag) };
    ...
    std::transform(files.begin(), files.end(), std::back_inserter(files_backup),
                   [](const QString &file) {
                       return DAEMONPTAGDM_NAMESPACE::restoreEscapedEnSkim(file);
                   });
...
QList<QString> DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::GetSameTagsOfDiffFiles, QList<QString>>(const QMap<QString, QList<QString>> &filesAndTags)
{
    ...
    std::transform(totalTagsNames.begin(), totalTagsNames.end(), std::back_inserter(total_tags_names_backup),
                   [](const QString &tag_name) {
                       return DAEMONPTAGDM_NAMESPACE::restoreEscapedEnSkim(tag_name);
                   });
...
QMap<QString, QVariant> DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::GetAllTags, QMap<QString, QVariant>>(const QMap<QString, QList<QString>> &filesAndTags)
{
    ...
    if (QFileInfo::exists("/home")) {
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::GetAllTags) };
        this->connectToShareSqlite("/home", ".__main.db");
        ...
        if (sqlDatabasePtr && sqlDatabasePtr->open()) {
            ...
            if (sql_query.exec(range.first->second)) {
                ...
                while (sql_query.next()) {
                    ...
                    tag_name = DAEMONPTAGDM_NAMESPACE::restoreEscapedEnSkim(tag_name);
                    tag_color = DAEMONPTAGDM_NAMESPACE::restoreEscapedEnSkim(tag_color);   //###: color can not contain skim.
...
QMap<QString, QVariant> DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::GetTagColor, QMap<QString, QVariant>>(const QMap<QString, QList<QString>> &fileAndTags)
{
    ...
    if (QFileInfo::exists("/home") && !fileAndTags.isEmpty()) {
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::GetTagColor) };
        this->connectToShareSqlite("/home", ".__main.db");
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::ChangeTagColor, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    ...
    if (QFileInfo::exists("/home") && !filesAndTags.isEmpty()) {
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::ChangeTagColor) };
        this->connectToShareSqlite("/home", ".__main.db");
...
bool DSqliteHandle::execSqlstr<DSqliteHandle::SqlType::BeforeTagFiles, bool>(const QMap<QString, QList<QString>> &filesAndTags)
{
    if (QFileInfo::exists("/home") && !filesAndTags.isEmpty()) {
        std::pair<std::multimap<DSqliteHandle::SqlType, QString>::const_iterator,
                  std::multimap<DSqliteHandle::SqlType, QString>::const_iterator>
                range { SqlTypeWithStrs.equal_range(DSqliteHandle::SqlType::BeforeTagFiles) };
        this->connectToShareSqlite("/home", ".__main.db");
        ...
        if (sqlDatabasePtr && sqlDatabasePtr->open() && sqlDatabasePtr->transaction()) {
                ...
                if (sql_query.exec(counting)) {
                    ...
                    if (sql_query.next()) {
                        ...
                        if (number == 0) {
                            ...
                            newAddedTags.push_back(c_beg.key());
                        }
                    }
                }
            }
            ...
            if (!newAddedTags.isEmpty()) {
                emit addNewTags(QVariant { newAddedTags });
                newAddedTags.clear();
            }
