...
public:
    ...
    static QPair<QString, QString> getMountPointOfFile(QUrl url, std::unique_ptr<std::map<QString, std::multimap<QString, QString>>> &partionsAndMountPoints);
...
private slots:
    void onMountAdded(const QString &deviceId, const QString &mountPoint);
    void onMountRemoved(const QString &deviceId);
...
private:
    ...
    {
        if (sqlDatabasePtr && sqlDatabasePtr->isOpen()) {
    ...
    std::unique_ptr<std::map<QString, std::multimap<QString, QString>>> partionsOfDevices { nullptr };
    std::unique_ptr<QSqlDatabase> sqlDatabasePtr { nullptr };
    std::atomic<bool> flag { false };
    std::mutex mutex {};

    QList<QString> newAddedTags {};
};
