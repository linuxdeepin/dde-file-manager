...
void DMInitEncryptWorker::run()
{
    if (!CryptSetupCompabilityHelper::instance()->initWithPreProcess()) {
        qCritical() << "Key function not provided by cryptsetup, cannot encrypt with overlay method";
        return;
    }

    qInfo() << "==> DMInitEncryptWorker::run()";

    const QString &devPath = m_args.value(disk_encrypt::encrypt_param_keys::kKeyDevice).toString();   // /dev/dm-0
    qInfo() << "About to encrypt overlay device:" << devPath;
    ...
    qDebug() << "Device names - top:" << topName << ", mid:" << midName << ", unlock:" << unlockName;

    QString phyPath = dm_setup_helper::findHolderDev(devPath);
    qInfo() << "Physical device path:" << phyPath;
    ...
    qInfo() << "Crypttab item inserted for unlock device:" << unlockName;
    ...
    if (r < 0) {
        qCritical() << EInitEnc << "device:" << phyPath << "error:" << r;
        job_file_helper::removeJobFile(jobArgs.jobFile);
        setExitCode(r);
        return;
    }
    qInfo() << "Overlay encryption initialized successfully, device:" << phyPath;

    r = crypt_setup::csActivateDeviceByVolume(phyPath, unlockName, proc.volumeKey);
    if (r < 0) {
        qWarning() << "Cannot activate device by volume key, trying passphrase, device:" << phyPath << "error:" << r;
        RetOnFail(crypt_setup::csActivateDevice(phyPath, unlockName), EActive + ", device: " + phyPath);
    }
    qInfo() << "Overlay device activated successfully, physical:" << phyPath << ", unlock:" << unlockName;
...
{
    qDebug() << "==> DMInitEncryptWorker::detachPhyDevice()";
    if (argc < 3) {
        qCritical() << "Parameter error, argc:" << argc << ", argv[0]:" << *argv;
        return -disk_encrypt::kErrorUnknown;
    }

    QString dev(argv[0]), topName(argv[1]), midName(argv[2]);
    qDebug() << "Detaching physical device:" << dev << ", top:" << topName << ", mid:" << midName;

    auto devSize = blockdev_helper::devBlockSize(dev);
    qDebug() << "Device size:" << devSize;
    ...
    qInfo() << "Starting suspend device:" << topName;
    ...
    do {
        ...
        if (r != 0) {
            qWarning() << "Suspend device failed, device:" << topName << "error:" << r;
            break;
        }

        // physical device already detached.
        qInfo() << "Physical device detached successfully";
    ...
    qWarning() << "dmsetup failed, reloaded to initial status, device:" << topName << "error:" << r;
    return r;
}
