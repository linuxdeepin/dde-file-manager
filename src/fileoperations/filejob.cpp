...
std::function<void (const ProgressInfo *)> udProgressCbWrapper;
extern "C" void udProgressCb(const ProgressInfo *info) {
    udProgressCbWrapper(info);
}
...
typedef char *(*uburn_get_last_err)();
...
static uburn_get_last_err       ub_get_last_err = nullptr;
...
public:
    LibHelper() {
        ...
        ub_get_last_err = reinterpret_cast<uburn_get_last_err>(lib.resolve("burn_get_last_error"));
        funcsLoaded &= (ub_get_last_err != nullptr);
...
/////////////////////////////
    ...
    if (pid == 0) { // child process, do real burn here
        ...
        udProgressCbWrapper = std::bind(executor_cb, std::placeholders::_1, this);
        ub_init();
        ub_regi_cb(udProgressCb);
        ub_show_verbose();
        ...
        buf[0] = ret == 0 ? '0' : '1';
        strncpy(buf + 1, ub_get_last_err(), BUFFERSIZE - 1);
