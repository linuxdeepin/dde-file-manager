...
public:
    ...
    explicit DAbstractFileInfo(const QUrl &url, bool hasCache = true);
    explicit DAbstractFileInfo(const QUrl &url, const QMimeType &mimetype);
    virtual ~DAbstractFileInfo();

    static const DAbstractFileInfoPointer getFileInfo(const QUrl &fileUrl);
    ...
    virtual QUrl symLinkTarget() const;
    virtual QUrl rootSymLinkTarget() const;
    ...
    virtual QList<QUrl> parentUrlList() const;
...
//    virtual quint8 supportViewMode() const;
    ...
    /// Return absoluteFilePath + "/" + fileName if is dir; otherwise return QUrl()
    virtual QUrl getUrlByChildFileName(const QString &fileName) const;

    virtual QUrl mimeDataUrl() const;
    ...
    virtual QUrl goToUrlWhenDeleted() const;
...
protected:
    ...
    Q_DECL_DEPRECATED_X("!!!!!!!!!!!!!!!!!!!!!!!") virtual void setUrl(const QUrl &url);
