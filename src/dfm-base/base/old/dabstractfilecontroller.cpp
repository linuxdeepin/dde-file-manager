...
QUrl DefaultDiriterator::next()
{
    return m_children.at(++m_current)->url();
}
...
QUrl DefaultDiriterator::url() const
{
    return m_children.at(m_current)->url();
}
...
QList<QUrl> DAbstractFileController::moveToTrash(const QSharedPointer<DFMMoveToTrashEvent> &event) const
{
//    event->ignore();
...
QList<QUrl> DAbstractFileController::pasteFile(const QSharedPointer<DFMPasteEvent> &event) const
{
//    event->ignore();
...
//        QStringList urls{QStringList() << QUrl.toLocalFile()};
...
//    }
//    return DDesktopServices::showFileItem(QUrl);
}
...
DFileDevice *DAbstractFileController::createFileDevice(const QSharedPointer<DFMUrlBaseEvent> &event) const
{
//    event->ignore();
...
DFileHandler *DAbstractFileController::createFileHandler(const QSharedPointer<DFMUrlBaseEvent> &event) const
{
//    event->ignore();
...
DStorageInfo *DAbstractFileController::createStorageInfo(const QSharedPointer<DFMUrlBaseEvent> &event) const
{
//    event->ignore();
