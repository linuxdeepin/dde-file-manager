...
QMap<QUrl, DAbstractFileInfo *> DAbstractFileInfoPrivate::urlToFileInfoMap;
QReadWriteLock *DAbstractFileInfoPrivate::urlToFileInfoMapLock = new QReadWriteLock();

DAbstractFileInfoPrivate::DAbstractFileInfoPrivate(const QUrl &url, DAbstractFileInfo *qq)
    : q_ptr(qq)
    , fileUrl(url)
{
    //###(zccrs): 只在主线程中开启缓存，防止不同线程中持有同一对象时的竞争问题
    if ((url.isValid() && (QThread::currentThread()) &&  qApp && qApp->thread() && QThread::currentThread() == qApp->thread())) {
...
void DAbstractFileInfoPrivate::setUrl(const QUrl &url, bool hasCache)
{
    if (url == fileUrl) {
        return;
    }
...
DAbstractFileInfo *DAbstractFileInfoPrivate::getFileInfo(const QUrl &fileUrl)
{
...
DAbstractFileInfo::DAbstractFileInfo(const QUrl &url, bool hasCache)
    : d_ptr(new DAbstractFileInfoPrivate(url, this))
{

}

DAbstractFileInfo::DAbstractFileInfo(const QUrl &url, const QMimeType &mimetype)
    : d_ptr(new DAbstractFileInfoPrivate(url, this))
{
    Q_UNUSED(mimetype);
}
...
const DAbstractFileInfoPointer DAbstractFileInfo::getFileInfo(const QUrl &fileUrl)
{
...
QUrl DAbstractFileInfo::symLinkTarget() const
{
...
QUrl DAbstractFileInfo::rootSymLinkTarget() const
{
    DAbstractFileInfoPointer info =
            DFileService::instance()->createFileInfo(Q_NULLPTR, url());

    while (info->isSymLink()) {
        QUrl targetUrl = info->symLinkTarget();

        if (targetUrl == info->url()) {
            break;
        }

        info = DFileService::instance()->createFileInfo(Q_NULLPTR, targetUrl);
        if (!info) {
            return QUrl();
        }
    }
...
int DAbstractFileInfo::filesCount() const
{
    ...
    const DDirIteratorPointer &iterator =
            DFileService::instance()->createDirIterator(Q_NULLPTR, url(), QStringList(),
...
QUrl DAbstractFileInfo::parentUrl() const
{
    return DFMUrlRoute::urlParent(url());
}

QList<QUrl> DAbstractFileInfo::parentUrlList() const
{
    QList<QUrl> urlList;

    Q_UNUSED(isAncestorsUrl(QUrl(), &urlList));

    return urlList;
}

bool DAbstractFileInfo::isAncestorsUrl(const QUrl &url, QList<QUrl> *ancestors) const
{
    ...
    Q_FOREVER {
        ...
        const DAbstractFileInfoPointer &fileInfo =
                DFileService::instance()->createFileInfo(Q_NULLPTR, parentUrl);
    ...
    if (type == SpaceArea) {
        ...
        if (isDir() /*&& systemPathManager->isSystemPath(filePath())*/) {
            ...
            if (isDir()) {
                ...
                // if (DFileService::instance()->createFileInfo(nullptr, QUrl::fromBookMarkFile(url(), QString()))) {
                if (!isSymLink()) {
                    //                    if (Singleton<BookMarkManager>::instance()->checkExist(QUrl::fromBookMarkFile(url(), QString()))) {
            ...
            Q_FOREACH (QString id, LabelMenuItemIds) {
...
QList<int> DAbstractFileInfo::userColumnRoles() const
{
    ...
    static QList<int> userColumnRoles;
    //    userColumnRoles = QList<int>() << DFileSystemModel::FileDisplayNameRole
...
QUrl DAbstractFileInfo::redirectedFileUrl() const
{
    if (isSymLink()) {
...
bool DAbstractFileInfo::isEmptyFloder(const QDir::Filters &filters) const
{
    ...
    DDirIteratorPointer it = DFileService::instance()->createDirIterator(Q_NULLPTR, url(), QStringList(),
                                                                         filters,
                                                                         QDirIterator::NoIteratorFlags);
...
QUrl DAbstractFileInfo::getUrlByNewFileName(const QString &fileName) const
{
    QUrl url = url();
...
QUrl DAbstractFileInfo::getUrlByChildFileName(const QString &fileName) const
{
...
QUrl DAbstractFileInfo::mimeDataUrl() const
{
    return url();
}
...
QUrl DAbstractFileInfo::goToUrlWhenDeleted() const
{
    QUrl parentUrl;

    Q_FOREACH (const QUrl &url, parentUrlList()) {
        parentUrl = url;
    ...
    return parentUrl.isValid() ? parentUrl : QUrl::fromLocalFile(QDir::homePath());
}
...
bool DAbstractFileInfo::canDrop() const
{
    ...
    do {
        const QUrl &targetUrl = info->symLinkTarget();
...
static QList<QAction *> getTemplateFileList()
{
    ...
    //                auto dfileinfo = DFileService::instance()->createFileInfo(nullptr, QUrl::fromLocalFile(fileInfo.filePath()));
...
QMap<MenuAction, QVector<MenuAction> > DAbstractFileInfo::subMenuActionList(MenuType type) const
{
    ...
    if (urlStr.isEmpty()) {
#ifdef QT_DEBUG
        qCritical() << url() << "scheme fileinfo seems doesn't implement toLocalFile() or implemented incorrectly.";
...
void DAbstractFileInfo::setUrl(const QUrl &url)
{
    Q_D(DAbstractFileInfo);
