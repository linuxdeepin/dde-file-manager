...
void onClipboardDataChanged()
{
    ...
    for (QUrl &_url : mimeData->urls()) {
        ...
        const DFMLocalFileInfo fileInfo(_url);
...
void DFMGlobal::setUrlsToClipboard(const QList<QUrl> &list, DFMGlobal::ClipboardAction action, QMimeData *mimeData)
{
    ...
    for (const QUrl &qurl : list) {
        ...
        const DAbstractFileInfo info(qurl);
...
//                DAbstractFileWatcher::ghostSignal(QUrl.parentUrl(), &DAbstractFileWatcher::fileAttributeChanged, QUrl);
//            }

//            // for tag watcher
//            const QUrl &file_url = QUrl::fromUserTaggedFile(tag_name, QString());

//            DAbstractFileWatcher::ghostSignal(QUrl(TAG_ROOT), &DAbstractFileWatcher::fileAttributeChanged, file_url);
//        }
//    });
...
//            QUrl QUrl = QUrl::fromLocalFile(i.key());
//            DAbstractFileWatcher::ghostSignal(QUrl.parentUrl(), &DAbstractFileWatcher::fileAttributeChanged, QUrl);

//            // for tag watcher
//            for (const QString &tag : i.value()) {
//                const QUrl &parent_url = QUrl::fromUserTaggedFile(tag, QString());
//                const QUrl &file_url = QUrl::fromUserTaggedFile(tag, i.key());
...
//            DAbstractFileWatcher::ghostSignal(QUrl(TAG_ROOT), &DAbstractFileWatcher::subfileCreated, QUrl::fromUserTaggedFile(tag, QString()));
//        }
//    });
//    connect(TagManager::instance(), &TagManager::deleteTag, [](const QList<QString> &new_tags) {
//        for (const QString &tag : new_tags) {
//            DAbstractFileWatcher::ghostSignal(QUrl(TAG_ROOT), &DAbstractFileWatcher::fileDeleted, QUrl::fromUserTaggedFile(tag, QString()));
//        }
//    });
...
//            const QUrl &old_url = QUrl::fromUserTaggedFile(i.key(), QString());
//            const QUrl &new_url = QUrl::fromUserTaggedFile(i.value(), QString());

//            DAbstractFileWatcher::ghostSignal(QUrl(TAG_ROOT), &DAbstractFileWatcher::fileMoved, old_url, new_url);
//        }
//    });
}
...
//        const QUrl &QUrl = QUrl::fromLocalFile(filePath);

//        const DAbstractFileInfoPointer &info = DFileService::instance()->createFileInfo(nullptr, QUrl);

//        if (!info)
//            return;

//        DAbstractFileWatcher::ghostSignal(info->parentUrl(), &DAbstractFileWatcher::fileAttributeChanged, QUrl);
//    });
}
...
bool DFMGlobal::isDesktopFile(const QUrl &url)
{
    return url.toLocalFile().endsWith(".desktop");
}

bool DFMGlobal::isTrashDesktopFile(const QUrl &url)
{
...
bool DFMGlobal::isComputerDesktopFile(const QUrl &url)
{
//    if (isDesktopFile(url)) {
//        DesktopFile df(url.toLocalFile());
...
bool DFMGlobal::isTrashDesktopFileUrl(const QUrl &url)
{
...
bool DFMGlobal::isComputerDesktopFileUrl(const QUrl &url)
{
...
void DFMGlobal::showMultiFilesRenameDialog(const QList<QUrl> &selectedFiles)
{
//    dialogManager->showMultiFilesRenameDialog(selectedFiles);
}

void DFMGlobal::showFilePreviewDialog(const QList<QUrl> &selectUrls, const QList<QUrl> &entryUrls)
{
//    Q_EMIT fileSignalManager->requestShowFilePreviewDialog(selectUrls, entryUrls);
}

void DFMGlobal::showPropertyDialog(QObject *const sender, const QList<QUrl> &selectedFiles)
{
