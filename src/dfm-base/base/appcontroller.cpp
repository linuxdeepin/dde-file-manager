...
QPair<QUrl, quint64> AppController::selectionAndRenameFile;
QPair<QUrl, quint64> AppController::selectionFile;
QPair<QList<QUrl>, quint64> AppController::multiSelectionFilesCache;
...
//{
//    const QList<QUrl> &urls = event->urlList();
//    if (urls.isEmpty()) {
//        return;
//    }
...
//                u = QUrl::fromLocalFile(u.path());
//            } else if (u.scheme() == SEARCH_SCHEME) { //搜索结果也存在右键批量打卡不成功的问题，这里做类似处理
//                u = u.searchedFileUrl();
//            } else if (u.scheme() == BURN_SCHEME) {
//                DAbstractFileInfoPointer info = DFileService::instance()->createFileInfo(event->sender(), u);
//                if (!info)
//                    continue;
...
//    DAbstractFileInfoPointer fi = DFileService::instance()->createFileInfo(event->sender(), fileUrl);
...
//    const QSharedPointer<DFMUrlListBaseEvent> &e =
//            dMakeEventPointer<DFMUrlListBaseEvent>(event->sender(), QList<QUrl>() << newUrl);
...
//    const QSharedPointer<DFMUrlListBaseEvent> newEvent(new DFMUrlListBaseEvent(event->sender(), QList<QUrl>() << newUrl));
...
//{
//    Q_EMIT fileSignalManager->requestShowOpenWithDialog(DFMUrlBaseEvent(event->sender(), event->url()));
//}

////新加app打开多个url
//void AppController::actionOpenFilesWithCustom(const QSharedPointer<DFMUrlListBaseEvent> &event)
//{
//    Q_EMIT fileSignalManager->requestShowOpenFilesWithDialog(DFMUrlListBaseEvent(event->sender(), event->urlList()));
//}
...
//        DFileService::instance()->openFileLocation(event->sender(), url);
//    }
//}
...
//{
//    DFileService::instance()->compressFiles(event->sender(), event->urlList());
//}

//void AppController::actionDecompress(const QSharedPointer<DFMUrlListBaseEvent> &event)
//{
//    DFileService::instance()->decompressFile(event->sender(), event->urlList());
//}

//void AppController::actionDecompressHere(const QSharedPointer<DFMUrlListBaseEvent> &event)
//{
//    DFileService::instance()->decompressFileHere(event->sender(), event->urlList());
//}

//void AppController::actionCut(const QSharedPointer<DFMUrlListBaseEvent> &event)
//{
//    DFileService::instance()->writeFilesToClipboard(event->sender(), DFMGlobal::CutAction, event->urlList());

//}

//void AppController::actionCopy(const QSharedPointer<DFMUrlListBaseEvent> &event)
//{
//    DFileService::instance()->writeFilesToClipboard(event->sender(), DFMGlobal::CopyAction, event->urlList());
//}
...
//        DFileService::instance()->pasteFileByClipboard(event->sender(), event->url());
//    });
//}

//void AppController::actionRename(const QSharedPointer<DFMUrlListBaseEvent> &event)
//{
//    QList<QUrl> urlList{ event->urlList() };
...
//        Q_EMIT fileSignalManager->requestRename(*singleFileEvent);

//    } else { //###: for more than one file.
//        Q_EMIT fileSignalManager->requestMultiFilesRename(*event);
//    }
//}

//void AppController::actionBookmarkRename(const QSharedPointer<DFMUrlBaseEvent> &event)
//{
//    Q_EMIT fileSignalManager->requestBookmarkRename(*event.data());
//}

//void AppController::actionBookmarkRemove(const QSharedPointer<DFMUrlBaseEvent> &event)
//{
//    DFileService::instance()->removeBookmark(event->sender(), event->url());
//}
...
//    if (!act)
//        return;
//    QList<QUrl> urlList = QUrl::fromStringList(act->property("urlList").toStringList());
////    DFileService::instance()->sendToBluetooth(urlList);
//}

//void AppController::actionAddToBookMark(const QSharedPointer<DFMUrlBaseEvent> &event)
//{
//    DFileService::instance()->addToBookmark(event->sender(), event->url());
//}
...
//    DFileService::instance()->mkdir(event->sender(), FileUtils::newDocumentUrl(info, tr("New Folder"), QString()));
//}

//void AppController::actionSelectAll(quint64 winId)
//{
//    Q_EMIT fileSignalManager->requestViewSelectAll(static_cast<int>(winId));
//}
...
//    bool ret = DFileService::instance()->deleteFiles(sender, list);
...
//    //            if (f->path().length()) {
//    //                this->actionOpen(dMakeEventPointer<DFMUrlListBaseEvent>(event->sender(), QList<QUrl>() << QUrl::fromLocalFile(f->path())));
//    //            }
//    //        }
//    //        gioproc->deleteLater();
//    //    });
...
//    ////        Q_EMIT fileSignalManager->requestAsynAbortJob(fi->redirectedFileUrl());

//    //        if (fi->suffix() == SUFFIX_UDISKS) {
//    //            //在主线程去调用unmount时如果弹出权限认证窗口，会导致文管界面挂起，
//    //            //在关闭窗口特效情况下，还会出现文管界面绘制不刷新出现重影的情况
//    //            //把unmount相关操作移至子线程
//    //            Q_EMIT doUnmount(fi->extraProperties()["udisksblk"].toString());
//    //        } else if (fi->suffix() == SUFFIX_GVFSMP) {
//    //            QString path = fi->extraProperties()["rooturi"].toString();
//    //            if (path.isEmpty()) {
//    //                //FIXME(zccrs): 为解决，无法访问smb共享地址时，点击卸载共享目录，创建的fileinfo拿到的path为""，所有就没办法umount，临时解决方案，重QUrl中获取
...
//    //        QString dev = fileUrl.query(QUrl::FullyEncoded);
...
//{
//    DFileService::instance()->restoreFile(event->sender(), QList<QUrl>() << event->url());
//}
...
//    if (fileUrl.scheme() == DFMROOT_SCHEME) {
//        DAbstractFileInfoPointer fi = DFileService::instance()->createFileInfo(this, fileUrl);

//        // Only optical media has eject option, should terminate all oprations while ejecting...
//        Q_EMIT fileSignalManager->requestAsynAbortJob(fi->redirectedFileUrl());
//        //        QtConcurrent::run([fi]() {
//        //            qDebug() << fi->url().path();
//        //            QString strVolTag = fi->url().path().remove("/").remove(".localdisk"); // /sr0.localdisk 去头去尾取卷标
...
//        //        });
//    } else {
//        //        deviceListener->eject(fileUrl.query(QUrl::FullyEncoded));
//    }
//}
...
//    //        //在关闭窗口特效情况下，还会出现文管界面绘制不刷新出现重影的情况
//    //        //把unmount相关操作移至子线程
//    //        Q_EMIT doSaveRemove(fi->extraProperties()["udisksblk"].toString());
//    //    } else {
//    //        QString unix_device = fileUrl.query(QUrl::FullyEncoded);
...
//{
//    for (const QUrl &url : event->urlList()) {
//        DFileService::instance()->openInTerminal(event->sender(), url);
//    }
//}
...
//        QUrl realTargetUrl = url;

//        //consider symlink file that links to trash/computer desktop files
//        const DAbstractFileInfoPointer &info = DFileService::instance()->createFileInfo(event->sender(), url);
...
//        //                url = QUrl(DFMROOT_ROOT + newDev + "." + SUFFIX_UDISKS);
//        //                //fix lvm分区下的逻辑目录打开属性需要对url做转换
//        //                if (dev.contains("/mapper/")) {
//        //                    QList<DAbstractFileInfoPointer> ch = DFileService::instance()->getChildren(this, QUrl(DFMROOT_ROOT), {}, nullptr);
//        //                    for (auto chi : ch) {
...
//        //        if (info && info->scheme() == BURN_SCHEME && info->url().burnFilePath().contains(QRegularExpression("^/*$"))) {
//        //            url = QUrl(DFMROOT_ROOT + info->url().burnDestDevice().mid(QString("/dev/").length()) + "." + SUFFIX_UDISKS);
//        //        }
...
//    if (urlList.first() == QUrl::fromTrashFile("/")) {
//        Q_EMIT fileSignalManager->requestShowTrashPropertyDialog(DFMUrlBaseEvent(event->sender(), urlList.first()));
//    } else if (urlList.first() == QUrl::fromComputerFile("/")) {
//        Q_EMIT fileSignalManager->requestShowComputerPropertyDialog(DFMUrlBaseEvent(event->sender(), urlList.first()));
//    } else {
//        Q_EMIT fileSignalManager->requestShowPropertyDialog(DFMUrlListBaseEvent(event->sender(), urlList));
//    }
//}
...
//{
//    Q_EMIT fileSignalManager->aboutToCloseLastActivedWindow(static_cast<int>(winId));
//}
...
//{
//    Q_EMIT fileSignalManager->requestShowShareOptionsInPropertyDialog(*event.data());
//}

//void AppController::actionUnShare(const QSharedPointer<DFMUrlBaseEvent> &event)
//{
//    DFileService::instance()->unShareFolder(event->sender(), event->url());
//}
...
//{
//    Q_EMIT fileSignalManager->requestSearchCtrlL(winId);
//}

//void AppController::actionctrlF(quint64 winId)
//{
//    Q_EMIT fileSignalManager->requestSearchCtrlF(winId);
//}
...
//{
//    QString path;
//    DAbstractFileInfoPointer fi = DFileService::instance()->createFileInfo(event->sender(), event->url());
...
//    //        obj.insert("protocol", QUrl(smbObj.value("id").toString()).scheme());
...
//        QList<QUrl> fileUrls = qvariant_cast<QList<QUrl>>(action->property("urls"));
...
//    } else {
//        QUrl fileUrl = qvariant_cast<QUrl>(action->property("url"));
//        DFileService::instance()->openFileByApp(this, app, fileUrl);
//    }
//}
...
//    QUrl targetUrl = QUrl(action->property("mounted_root_uri").toString());
//    QList<QUrl> urlList = QUrl::fromStringList(action->property("urlList").toStringList());
...
//            DAbstractFileInfoPointer fi = DFileService::instance()->createFileInfo(nullptr, u);
//            if (fi)
...
//    //        }
//    //        QUrl tempTargetUrl = QUrl::fromLocalFile(strCachePath);
//    //        DFileService::instance()->pasteFile(action, DFMGlobal::CopyAction, tempTargetUrl, urlList);
//    //    } else { // other: usb storage and so on
//    DFileService::instance()->pasteFile(action, DFMGlobal::CopyAction, targetUrl, urlList);
//    //    }
//}
...
//    QList<QUrl> urlList = QUrl::fromStringList(action->property("urlList").toStringList());
//    for (QUrl &u : urlList) {
//        DAbstractFileInfoPointer fi = DFileService::instance()->createFileInfo(sender(), u);
...
//    //            QUrl dest = QUrl::fromBurnFile(QString(blkd->device()) + "/" BURN_SEG_STAGING "/");
//    //            DFileService::instance()->pasteFile(action, DFMGlobal::CopyAction, dest, urlList);
//    //            break;
//    //        }
//    //    }
//}
...
//void AppController::showTagEdit(const QRect &parentRect, const QPoint &globalPos, const QList<QUrl> &fileList)
//{
...
void AppController::actionSetLabel(const DFMEvent &event)
{
    if (FileManagerLibrary::instance()->isCompletion()) {
        std::string path = event.url().toLocalFile().toStdString();
...
QString AppController::createFile(const QString &sourceFile, const QString &targetDir, const QString &baseFileName, WId windowId)
{
    ...
    //    AppController::selectionAndRenameFile = qMakePair(QUrl::fromLocalFile(targetFile), windowId);

    //    if (DFileService::instance()->touchFile(WindowManager::getWindowById(windowId), QUrl::fromLocalFile(targetFile))) {
...
void UnmountWorker::doUnmount(const QString &blkStr)
{
    ...
    //        Q_EMIT unmountResult(tr("The device was not safely unmounted"), manager.getErrorMsg());
}
...
void UnmountWorker::doSaveRemove(const QString &blkStr)
{
    ...
    //        Q_EMIT unmountResult(tr("The device was not safely removed"), manager.getErrorMsg());
}
