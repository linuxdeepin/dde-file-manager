...
class DFMInfoCachesManagerPrivate;
...
class DFMInfoCachesManager : public QObject
{
    Q_OBJECT
    Q_DECLARE_PRIVATE_D(qGetPtrHelper(d_ptr), DFMInfoCachesManager)
    friend DFMInfoFactory;
    friend ReFreshThread;
public:
    explicit DFMInfoCachesManager(QObject *parent = nullptr);
    virtual ~DFMInfoCachesManager();
private:
    /**
     * @brief instance 获取DFileInfoManager的对象
     * @return DFileInfoManager的对象
     */
    static DFMInfoCachesManager &instance();
    /**
...
protected:
    QScopedPointer<DFMInfoCachesManagerPrivate> d_ptr;
};
