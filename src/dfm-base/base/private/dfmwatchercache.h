...
class DFMWatcherCache : public QObject
{
    Q_OBJECT
    Q_DECLARE_PRIVATE_D(qGetPtrHelper(d_ptr), DFMWatcherCache)
    QScopedPointer<DFMWatcherCachePrivate> d_ptr;
    friend DFMWacherFactory;
    friend DFMInfoCache;
public:
    explicit DFMWatcherCache(QObject *parent = nullptr);
    virtual ~DFMWatcherCache();
...
private:
     ...
     * @return DFMWatcherCache& 返回全局的文件监视器的缓存管理模块实例
     */
    static DFMWatcherCache& instance();
