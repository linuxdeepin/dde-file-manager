...
#include "base/private/dfminfocache_p.h"
...
Q_GLOBAL_STATIC(DFMInfoCache,_InfoCacheManager)
...
{
    DAbstractFileInfoPointer fileinfo = DFMInfoCache::instance().getCacheInfo(url);
    if (!fileinfo) {
...
void ReFreshThread::run()
{
    ...
    while(itr != itrEnd) {
        DAbstractFileInfoPointer fileinfo = DFMInfoCache::instance().getCacheInfo(itr.key());
        if (!fileinfo)
            continue;
        if (m_bStop)
            return;
...
DFMInfoCachePrivate::DFMInfoCachePrivate(DFMInfoCache *qq)
    : q_ptr(qq)
{
...
DFMInfoCachePrivate::~DFMInfoCachePrivate()
{
    if (m_refreshThread) {
...
void DFMInfoCachePrivate::updateSortByTimeCacheUrlList(const QUrl &url)
{
...
DFMInfoCache::DFMInfoCache(QObject *parent)
    : QObject(parent)
    , d_ptr(new DFMInfoCachePrivate(this))
{
    connect(&d_ptr->m_removeTimer, &QTimer::timeout, this, &DFMInfoCache::timeRemoveCache);
    d_ptr->m_removeTimer.setInterval(CACHE_REMOVE_TIME);
    d_ptr->m_removeTimer.start();
    connect(&d_ptr->m_needRemoveTimer, &QTimer::timeout, this, &DFMInfoCache::timeNeedRemoveCache);
    d_ptr->m_needRemoveTimer.start(1000);
    d_ptr->m_needRemoveTimer.setInterval(CACHE_REMOVE_TIME);
}

DFMInfoCache::~DFMInfoCache()
{
    Q_D(DFMInfoCache);
...
DFMInfoCache &DFMInfoCache::instance()
{
    return *_InfoCacheManager;
}

DAbstractFileInfoPointer DFMInfoCache::getCacheInfo(const QUrl &url)
{
    Q_D(DFMInfoCache);
...
void DFMInfoCache::cacheInfo(const QUrl &url, const DAbstractFileInfoPointer &info)
{
    Q_D(DFMInfoCache);
    ...
    if (watcher) {
        connect(watcher.data(), &DAbstractFileWatcher::fileDeleted, this,  &DFMInfoCache::refreshFileInfo);
        connect(watcher.data(), &DAbstractFileWatcher::fileModified, this,  &DFMInfoCache::refreshFileInfo);
        connect(watcher.data(), &DAbstractFileWatcher::fileMoved, this,  &DFMInfoCache::refreshFileInfo);
    }
...
void DFMInfoCache::removeCacheInfo(const QUrl &url)
{
    Q_D(DFMInfoCache);
    // 断开信号连接
    QSharedPointer<DAbstractFileWatcher>  watcher = DFMWacherFactory::instance().create<DAbstractFileWatcher>(url);
    if (watcher) {
        ...
        DFMWatcherCache::instance().removCacheWatcher(url);
    }
...
void DFMInfoCache::refreshFileInfo(const QUrl &url)
{
    Q_D(DFMInfoCache);
...
void DFMInfoCache::timeNeedRemoveCache()
{
    Q_D(DFMInfoCache);
...
void DFMInfoCache::timeRemoveCache()
{
    Q_D(DFMInfoCache);
