...
//#define // namespace DFM_NAMESPACE {
//#define // }
//#define  using namespace DFM_NAMESPACE;
...
public:
    ...
    static bool isTrashDesktopFileUrl(const QUrl &url);
    static bool isComputerDesktopFileUrl(const QUrl &url);
    ...
    static void showMultiFilesRenameDialog(const QList<QUrl> &selectedFiles);

    static void showFilePreviewDialog(const QList<QUrl> &selectUrls, const QList<QUrl> &entryUrls);

    ///###: this function is special, if select files is more than 16 show a dialog for multiple files.
    static void showPropertyDialog(QObject *const sender, const QList<QUrl> &selectedFiles);
