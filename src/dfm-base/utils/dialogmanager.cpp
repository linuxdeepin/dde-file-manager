...
int DialogManager::showRunExcutableScriptDialog(const QUrl &url)
{
    ...
    const QString &fileDisplayName = info->displayInfo(DisPlay::kFileDisplayName);
...
int DialogManager::showDeleteFilesDialog(const QList<QUrl> &urlList)
{
    ...
    if (isLocalFile) {
        if (urlList.size() == 1) {
            ...
            fileName = f.displayInfo(DisPlay::kFileDisplayName);
        } else {
...
int DialogManager::showNormalDeleteConfirmDialog(const QList<QUrl> &urls)
{
    ...
    if (urlFirst.isLocalFile()) {   // delete local file
        if (urls.size() == 1) {
            ...
            d.setTitle(deleteFileName.arg(fm.elidedText(info->displayInfo(DisPlay::kFileDisplayName), Qt::ElideMiddle, NAME_MAX)));
        } else {
