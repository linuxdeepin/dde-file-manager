 ...
 * @param
 */
bool FileUtils::isAncestorUrl(const QUrl &ancestor, const QUrl &url)
{
    QUrl parent = url;
 ...
 */
qint64 FileUtils::singleDirSize(const QUrl &url)
{
    qint64 size = 0;
    ...
    if (targetInfo.exists()) {
        if (targetInfo.isDir()) {
            ...
            QFileInfoList entryInfoList = d.entryInfoList(QDir::AllEntries | QDir::System
                                                          ...
                                                          | QDir::Hidden);
            Q_FOREACH (QFileInfo file, entryInfoList) {
                if (file.isFile()) {
                    total += file.size();
                } else {
 ...
 * @return total size
 */
qint64 FileUtils::totalSize(const QList<QUrl> &files, qint32 &dirSize, qint32 &fileCount)
{
    qint64 total = 0;
    dirSize = 0;
    fileCount = 0;
...
qint64 FileUtils::totalSize(const QList<QUrl> &files, const qint64 &maxLimit, bool &isInLimit)
{
    qint64 total = 1;
    Q_FOREACH (QUrl url, files) {
        QFileInfo file = url.path();
...
bool FileUtils::openFile(const QString &filePath)
{
    ...
    //    DFMLocalFileInfo info(filePath);
    //    QString mimetype;
    //    if (info.size() == 0 && info.exists()) {
    //        mimetype = DMimeDatabase::mimeTypeForFile(info.fileName()).name();
    //    } else {
    ...
    //        // addToRecentFile(QUrl::fromLocalFile(filePath), mimetype);
...
bool FileUtils::openFiles(const QStringList &filePaths)
{
    ...
    //    //解决空文本文件转其他非文本格式时打开仍然是文本方式打开的问题
    //    //QString mimetype = getFileMimetype(filePath);
    //    DAbstractFileInfoPointer info = DFileService::instance()->createFileInfo(nullptr, QUrl("file://"  + filePath));
    //    QString mimetype;
    ...
    //        appAgrs << QUrl::fromLocalFile(tmp).toString();
    ...
    //        // addToRecentFile(QUrl::fromLocalFile(filePath), mimetype);
    //        for (const QString &tmp : rePath) {
    //            QString file_path = QUrl::fromLocalFile(tmp).toLocalFile();
    //            DesktopFile df(defaultDesktopFile);
...
bool FileUtils::openEnterFiles(const QStringList &filePaths)
{
    ...
    //    Q_FOREACH (const QString &filePath, rePath) {
    //        /*********************************************************/
    //        //解决空文本文件转其他非文本格式时打开仍然是文本方式打开的问题
    //        //QString mimetype = getFileMimetype(filePath);
    //        DAbstractFileInfoPointer info = DFileService::instance()->createFileInfo(nullptr, QUrl("file://"  + filePath));
    ...
    //            mimetype = getFileMimetype(filePath);
    //        }
    //        mimetypeinfo.insert(QUrl::fromLocalFile(filePath).toString(), mimetype);
    //        /*********************************************************/
    ...
    //            value << QUrl::fromLocalFile(filePath).toString();
    ...
    //        appAgrs << QUrl::fromLocalFile(tmp).toString();
    //    bool result = false;
    //    Q_FOREACH (const QString &defaultDesktopFile, openinfo.keys()) {
    ...
    //                QString filePath = QUrl::fromUserInput(tmp).toLocalFile();
    //                DesktopFile df(defaultDesktopFile);
...
bool FileUtils::launchAppByDBus(const QString &desktopFile, const QStringList &filePaths)
{
    ...
    //        AppController::instance()->startManagerInterface()->LaunchApp(desktopFile, static_cast<uint>(QX11Info::getTimestamp()), filePaths);
    //        return true;
    //    }
    return false;
}
...
bool FileUtils::openFilesByApp(const QString &desktopFile, const QStringList &filePaths)
{
    ...
    //            QString temFilePath = QUrl::fromUserInput(tmp).path();
...
//        return;
//    }
//    mkpath(DFMUrlRoute::urlParent(path));
//    DFileService::instance()->mkdir(nullptr, path);
}
...
//bool FileUtils::appendCompress(const QUrl &toUrl, const QList<QUrl> &fromUrlList)
//{
//    if (!fromUrlList.isEmpty()) {
//        QStringList arguments {toUrl.toLocalFile()};
//        Q_FOREACH (const QUrl &url, fromUrlList) {
