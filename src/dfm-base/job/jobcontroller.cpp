...
JobController::JobController(const QUrl &fileUrl, const DAbstractDirIteratorPointer &iterator, bool silent, QObject *parent)
    : QThread(parent)
    , m_silent(silent)
    , m_iterator(iterator)
    , m_fileUrl(fileUrl)
...
JobController::JobController(const QUrl &fileUrl, const QStringList &nameFilters,
...
//        Q_EMIT childrenUpdated(list);
//        Q_EMIT addChildrenList(list);
...
//        Q_EMIT childrenUpdated(fileInfoQueue);
//        Q_EMIT addChildrenList(fileInfoQueue);
//    }
