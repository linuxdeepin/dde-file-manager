...
//#include "localfile/dfileinfo.h"
//#include "localfile/dlocalfiledevice.h"
...
//    QUrl next() override
//    {
//        return QUrl::fromLocalFile(iterator.next());
//    }
...
//        QMimeType mimetype;
//        bool isdesktop = FileUtils::isDesktopFile(info);
...
//    QUrl url() const override
//    {
//        return QUrl::fromLocalFile(iterator.path());
//    }
...
//        return QUrl::fromLocalFile(currentFileInfo.absoluteFilePath());
//    }
...
//    QUrl url() const override
//    {
//        return QUrl::fromLocalFile(currentFileInfo.filePath());
//    }
...
//    QHash<QUrl, DAbstractFileInfoPointer> nextInofCached;
//};
...
//    //    QMimeType mimetype;
//    //    bool isdesktop = FileUtils::isDesktopFile(localFile);
...
//    //        const_cast<QUrl &>(fileUrl) = linkInfo->redirectedFileUrl();
//    //    }
...
//{
//    //    QList<QUrl> fileUrls = event->urlList();
//    //    QList<QUrl> packUrl;
...
//    //        QStringList args;
//    //        args << "-d";
//    //        Q_FOREACH (QUrl url, event->urlList()) {
...
//    //    DFMGlobal::setUrlsToClipboard(QUrl::toQList<QUrl>(urlList), event->action());
...
//{
//    //    const QUrl &olQUrl = event->fromUrl();
//    //    const QUrl &newUrl = event->toUrl();

//    //    QFile file(olQUrl.toLocalFile());
//    //    const QString &newFilePath = newUrl.toLocalFile();

//    //    const DAbstractFileInfoPointer &oldfilePointer = DFileService::instance()->createFileInfo(this, olQUrl);
...
//    //        QString filePath = olQUrl.toLocalFile();
...
//    //            DFMEventDispatcher::instance()->processEvent<DFMSaveOperatorEvent>(event, dMakeEventPointer<DFMRenameEvent>(nullptr, olQUrl, QUrl::fromLocalFile(path)));
//    //        }
//    //    } else {
//    //        if (newFilePath.contains("gvfs/mtp")) {
//    //            result = renameFileByGio(olQUrl, newUrl);
//    //        }
...
//    //        if (result) {
//    //            DFMEventDispatcher::instance()->processEvent<DFMSaveOperatorEvent>(event, dMakeEventPointer<DFMRenameEvent>(nullptr, newUrl, olQUrl));
...
////bool FileController::isExtDeviceJobCase(void *curJob, const QUrl &url) const
//>>>>>>> 6a9e2741bbae473a416407912337a7f3e86c9165
////{
////public:
////    DFMQDirIterator(const QString &path,
////                    const QStringList &nameFilters,
...
//////    if (!thisJob)
//////        return false;
//////    QList<QUrl> srcUrlList = thisJob->sourceUrlList();
//////    QUrl targetUrl = thisJob->targetUrl();
...
//////        Q_FOREACH (auto d, deviceListener->getDeviceList()) {
...
//////bool FileController::isDiscburnJobCase(void *curJob, const QUrl &url) const
//////{
//////    DFileCopyMoveJob *thisJob = static_cast<DFileCopyMoveJob *>(curJob);
...
//////    Q_FOREACH (QUrl oneUrl, srcUrlList) {
...
//////QList<QUrl> FileController::pasteFilesV2(const QSharedPointer<DFMPasteEvent> &event, DFMGlobal::ClipboardAction action, const QList<QUrl> &list, const QUrl &target, bool slient, bool force, bool bold) const
//////{
...
//////    if (!event.isNull() && !qvariant_cast<QList<QUrl>>(event->cutData()).isEmpty()) {
...
//////        {
//////            //线程启动传递源地址和目标地址
//////            connect(job.data(), &DFileCopyMoveJob::currentJobChanged, this, [this](const QUrl & from, const QUrl & to) {
//////                QMutex mutex;
//////                mutex.lock();
...
//////                if (sourceInfo->url() == targetInfo->url() ||
//////                        DStorageInfo::isSameFile(sourceInfo->url().path(), targetInfo->url().path())) {
...
//////            Q_EMIT job->currentJobChanged(sourceInfo ? sourceInfo->url() : QUrl(), targetInfo ? targetInfo->url() : QUrl(), true);
...
//////                // fix bug 62822 设置当前进度条已显示，来显示进度到100%
//////                fileJob->setProgressShow(true);
//////                Q_EMIT fileJob->currentJobChanged(currentJob.first, currentJob.second, false);
//////            }
...
//////    connect(fileSignalManager, &FileSignalManager::requestAsynAbortJob, thisJob, [thisJob, this](const QUrl & url) {
...
//////        QList<QUrl> targetUrlList = thisJob->targetUrlList();
//////        if (slient)
//////        {
//////            error_handle->deleteLater();
//////        } else
...
////void FileController::dealpasteEnd(const QList<QUrl> &list, const QSharedPointer<DFMPasteEvent> &event) const
////{
//////    QList<QUrl> valid_files = list;

//////    valid_files.removeAll(QUrl());
...
//////        } else {
//////            DFMEventDispatcher::instance()->processEvent<DFMSaveOperatorEvent>(event, dMakeEventPointer<DFMPasteEvent>(nullptr, DFMGlobal::CutAction, QUrl::fromLocalFile(targetDir), valid_files), true);
//////        }
//////    }
...
////    //    bool ok = !pasteFilesV2(nullptr, DFMGlobal::CutAction, event->fileUrlList(), QUrl(), event->silent(), event->force(), true).isEmpty();
...
//// */
//////QList<QUrl> FileController::moveToTrash(const QSharedPointer<DFMMoveToTrashEvent> &event) const
//////{
//////    FileJob job(FileJob::Trash);
//////    job.setWindowId(static_cast<int>(event->windowId()));
//////    dialogManager->addJob(&job);
//////    QList<QUrl> list = job.doMoveToTrash(event->urlList());
//////    dialogManager->removeJob(job.getJobId());

//////    // save event
//////    const QVariant &result = DFMEventDispatcher::instance()->processEvent<DFMGetChildrensEvent>(event->sender(), QUrl::fromTrashFile("/"),
...
//////    const QSet<QUrl> &source_files_set = event->urlList().toSet();
//////    const QSet<QUrl> &target_files_set = list.toSet();
//////    QList<QUrl> has_restore_files;

//////    for (const DAbstractFileInfoPointer &info : infos) {
//////        const QUrl &source_file = QUrl::fromLocalFile(static_cast<const TrashFileInfo *>(info.constData())->sourceFilePath());

//////        if (source_files_set.contains(source_file) && target_files_set.contains(info->mimeDataUrl())) {
//////            has_restore_files << info->url();
//////        }
//////    }
...
////static QList<QUrl> pasteFilesV1(const QSharedPointer<DFMPasteEvent> &event)
////{
////    //    const QList<QUrl> &urlList = event->urlList();
...
////    //        Q_EMIT fileSignalManager->requestShowNoPermissionDialog(noPermissionEvent);
////    //        return list;
////    //    }

////    //    if (event->action() == DFMGlobal::CutAction) {
////    //        QUrl parentUrl = QUrl::parentUrl(urlList.first());
...
////QList<QUrl> FileController::pasteFile(const QSharedPointer<DFMPasteEvent> &event) const
////{
...
////    //    if (ok) {
////    //        fileAdded(event->url());
////    //        DFMEventDispatcher::instance()->processEvent<DFMSaveOperatorEvent>(event, dMakeEventPointer<DFMDeleteEvent>(nullptr, QList<QUrl>() << event->url(), true));
////    //    } else {
////    //        // 创建文件夹失败，提示错误信息
...
////{
////    //    QUrl destUrl = event->url();

////    //    const DAbstractFileInfoPointer &p = DFileService::instance()->createFileInfo(nullptr, destUrl);
////    //    QUrl bookmarkUrl = QUrl::fromBookMarkFile(destUrl, p->fileDisplayName());
...
////    //            devStr = QUrl::fromDeviceId(info.device()).toString();
////    //        }
...
////{
//////    return DFileService::instance()->deleteFiles(nullptr, {QUrl::fromBookMarkFile(event->url(), QString())}, false);
////    return false;
////}
...
//////    bool result = file.link(event->toUrl().toLocalFile());
...
//////    int code = ::symlink(event->url().toLocalFile().toLocal8Bit().constData(),
...
////bool FileController::fileAdded(const QUrl &url) const
////{
////    // 华为平台特有的问题，inotify无法向mtp发送信号，因此采用以下模式在文件创建完成后来刷新模型
////    if (url.toLocalFile().contains("/mtp:")) {
////        return DAbstractFileWatcher::ghostSignal(DFMUrlRoute::urlParent(url), &DAbstractFileWatcher::subfileCreated, url);
////    }
////    return true;
////}
...
////    // file is exists
////    if (info) {
////        const_cast<FileDirIterator *>(this)->nextInofCached.insert(info->url(), info);
...
////QUrl FileDirIterator::url() const
////{
////    return iterator->url();
////}
...
////            (this)->nextInofCached.value(iterator->url());
////    if (newinfo) {
////        const_cast<FileDirIterator *>\
////                (this)->nextInofCached.remove(iterator->url());
////        return newinfo;
////    }
