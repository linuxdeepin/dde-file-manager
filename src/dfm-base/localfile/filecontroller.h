...
////    QList<QUrl> moveToTrash(const QSharedPointer<DFMMoveToTrashEvent> &event) const override;
//    QList<QUrl> pasteFile(const QSharedPointer<DFMPasteEvent> &event) const override;
...
//    DFileDevice *createFileDevice(const QSharedPointer<DFMUrlBaseEvent> &event) const override;
//    DFileHandler *createFileHandler(const QSharedPointer<DFMUrlBaseEvent> &event) const override;
////    DStorageInfo *createStorageInfo(const QSharedPointer<DFMUrlBaseEvent> &event) const override;
...
////    QList<QUrl> pasteFilesV2(const QSharedPointer<DFMPasteEvent> &event, DFMGlobal::ClipboardAction action, const QList<QUrl> &list, const QUrl &target, bool slient = false, bool force = false, bool bold = false) const;
//    //处理复制、粘贴和剪切(拷贝)结束后操作 fix bug 35855
//    void dealpasteEnd(const QList<QUrl> &lsit, const QSharedPointer<DFMPasteEvent> &event) const;
////    bool isExtDeviceJobCase(void *curJob, const QUrl &url) const;
////    bool isDiscburnJobCase(void *curJob, const QUrl &url) const;
//    bool fileAdded(const QUrl &url) const;
//};
