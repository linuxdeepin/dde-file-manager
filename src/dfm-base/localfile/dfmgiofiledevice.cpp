...
#include "dfmgiofiledevice.h"
#include "base/private/dabstractfiledevice_p.h"
#include "base/dabstractfilewatcher.h"
#include "localfile/private/dfmgiofiledevice_p.h"

DFMGIOFileDevicePrivate::DFMGIOFileDevicePrivate(DFMGIOFileDevice *qq)
...
DFMGIOFileDevicePrivate::~DFMGIOFileDevicePrivate()
{
...
DFMGIOFileDevice::DFMGIOFileDevice(const QUrl &url)
    : DAbstractFileDevice(url)
{
    setFileUrl(url);
}

DFMGIOFileDevice::DFMGIOFileDevice()
    : DAbstractFileDevice(*new DFMGIOFileDevicePrivate(this))
{
...
bool DFMGIOFileDevice::open(QIODevice::OpenMode mode)
{
    if (isOpen())
        return false;
    ...
    if ((mode & (WriteOnly | Append | Truncate))) {
        ...
        if (!exists) {
            DAbstractFileWatcher::ghostSignal(d->url, &DAbstractFileWatcher::subfileCreated, d->url);
...
bool DFMGIOFileDevice::setFileUrl(const QUrl &url)
{
    Q_ASSERT(!isOpen());
    DAbstractFileDevice::setFileUrl(url);
...
int DFMGIOFileDevice::handle() const
{
    return -1;
}

qint64 DFMGIOFileDevice::size() const
{
    Q_D(const DFMGIOFileDevice);
...
bool DFMGIOFileDevice::resize(qint64 size)
{
    Q_D(DFMGIOFileDevice);
...
bool DFMGIOFileDevice::isSequential() const
{
    Q_D(const DFMGIOFileDevice);
...
qint64 DFMGIOFileDevice::pos() const
{
    Q_D(const DFMGIOFileDevice);
...
bool DFMGIOFileDevice::seek(qint64 pos)
{
    Q_D(DFMGIOFileDevice);
...
bool DFMGIOFileDevice::syncToDisk(const SyncOperate &op)
{
    if(op == DefaultOperate) {
...
void DFMGIOFileDevice::closeWriteReadFailed(const bool bwrite)
{
    if (!isOpen())
        return;
...
void DFMGIOFileDevice::cancelAllOperate()
{
    Q_D(DFMGIOFileDevice);
    if (d->m_writeCancel)
...
qint64 DFMGIOFileDevice::readData(char *data, qint64 maxlen)
{
    Q_D(DFMGIOFileDevice);
...
qint64 DFMGIOFileDevice::writeData(const char *data, qint64 len)
{
    Q_D(DFMGIOFileDevice);
