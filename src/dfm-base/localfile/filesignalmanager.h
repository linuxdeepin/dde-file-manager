...
Q_SIGNALS:
    /* Q_EMIT usershare status changed*/
    void userShareCountChanged(const int count);

    // 在共享信息刷新后，请求刷新页面
    void requestRefreshFileModel(const QUrl &url);
    ...
    void requestOpenNewWindowByUrl(const QUrl &url, bool isAlwaysOpen);

    /*request active window*/
    void requestActiveWindow();

    /*request show url wrong dialog*/
    void requestShowUrlWrongDialog(const QUrl &url);
    ...
    void requestAbortJob(const QUrl &url);

    /*request abort copy/move job for DFileCopyMoveJob*/
    void requestAsynAbortJob(const QUrl &url);
    ...
    void requestShowFilePreviewDialog(const QList<QUrl> &selectUrls, const QList<QUrl> &entryUrls);
    ...
    void requestShowRestoreFailedDialog(const QList<QUrl> &urlList);
    ...
    void requestShowRestoreFailedSourceNotExist(const QList<QUrl> &urlList);
    ...
    /*rename a tag*/
    void requestRenameTag(const QUrl &oldTagUrl);
    ...
    // 请求重定向tab页
    void requestRedirectTabUrl(const QUrl &tabRootUrl, const QUrl &newUrl);

    // 请求关闭tab页
    void requestCloseTab(const QUrl &tabRootUrl);
    ...
    bool cdFolder(const QUrl &folderUrl);
#ifdef SW_LABEL
