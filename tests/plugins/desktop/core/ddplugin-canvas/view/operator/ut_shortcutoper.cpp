...
TEST_F(UT_ShortcutOper, keyPressed)
{
    ...
    FileOperatorProxy *fileBus = new FileOperatorProxy;
    CanvasProxyModel *proxymodel = new CanvasProxyModel;
    CanvasSelectionModel *model = new CanvasSelectionModel(proxymodel,nullptr);
    canvas->setSelectionModel(model);
    CanvasViewPrivate *pri = new CanvasViewPrivate(canvas);
   // canvas->d = QScopedPointer<CanvasViewPrivate>(pri);
    canvas->d->keySelector = new KeySelector(canvas);
    ...
    {
        QKeyEvent *key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Escape, Qt::NoModifier,"", true);
        EXPECT_TRUE(sc->keyPressed(key));
    }
    {
        QKeyEvent *key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Tab, Qt::NoModifier,"", true);
        {
            key->k = Qt::Key_F1;
            EXPECT_TRUE(sc->keyPressed(key));
        }
    ...
    }
    {
        ...
        QKeyEvent *key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Tab, Qt::KeypadModifier,"", true);
        {
    ...
    {
        QKeyEvent *key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Tab, Qt::ShiftModifier,"", true);
        {
            key->k = Qt::Key_Delete;
            EXPECT_TRUE(sc->keyPressed(key));
        }
    ...
    {
        QKeyEvent *key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Tab, Qt::ControlModifier,"", true);
        {
            key->k = Qt::Key_Minus;
            EXPECT_TRUE(sc->keyPressed(key));
        }
    ...
    {
        QKeyEvent *key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Tab, Qt::AltModifier,"", true);
        key->k = Qt::Key_M;
        EXPECT_TRUE(sc->keyPressed(key));
    }
    {
        QKeyEvent *key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Tab, (Qt::ControlModifier | Qt::ShiftModifier),"", true);
        key->k = Qt::Key_I;
        EXPECT_TRUE(sc->keyPressed(key));
    }
...
TEST_F(UT_ShortcutOper, showMenu)
{
    ...
    stub.set_lamda(&CanvasView::itemDelegate,[](){
        __DBG_STUB_INVOKE__
                CanvasView *view = new CanvasView();
                return new CanvasItemDelegate(view);
    });
    QModelIndex *rootIndex = new QModelIndex;
    CanvasProxyModel *proxymodel = new CanvasProxyModel;
    CanvasSelectionModel *model = new CanvasSelectionModel(proxymodel,nullptr);
