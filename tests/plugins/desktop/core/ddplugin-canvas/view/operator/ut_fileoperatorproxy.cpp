...
protected:
    virtual void SetUp() override
    {
        ...
        stub.set_lamda(&CanvasManager::views,[](){
            __DBG_STUB_INVOKE__
                   QSharedPointer<CanvasView> ptr(new  CanvasView());
...
{
    CanvasManager *manager = new CanvasManager(new QObject);
    ...
    CanvasIns->d->selectionModel = new CanvasSelectionModel(proxyModel,nullptr);
...
TEST_F(UT_FileOperatorProxy, callBackRenameFiles)
{
    ...
    CanvasIns->d->viewMap.insert("main",CanvasViewPointer(new CanvasView()));
...
{
    CanvasView *view = new CanvasView;
    file->pasteFiles(view,QPoint(1,1));
}
