...
TEST_F(UT_CanvasMenuScene, filterDisableAction)
{
    ...
    stub.set_lamda((fun_type)(&CanvasMenuScene::scene),[](QAction*){
        __DBG_STUB_INVOKE__
...
TEST_F(UT_CanvasMenuScene, triggered)
{
    ...
    CanvasManagerPrivate *managerP = new CanvasManagerPrivate(manager);
    manager->d = managerP;
    managerP->hookIfs = new CanvasManagerHook();

    menuScene->d->view = new CanvasView;
    menuScene->d->view->setModel(new CanvasProxyModel);
...
TEST_F(UT_CanvasMenuScene, iconSizeSubActions)
{
    menuScene->d->view = new CanvasView();
    menuScene->d->view->setModel(new CanvasProxyModel);
    menuScene->d->view->setItemDelegate(new CanvasItemDelegate(new CanvasView()));
    QMenu *parent = new QMenu;
