...
TEST(WindowFrame, rootWindows)
{
    ...
    stub.set_lamda(((QVariant (EventChannelManager::*)(const QString &, const QString &))
                   &EventChannelManager::push), [&callflag, &rets]
...
TEST(WindowFrame, buildBaseWindow)
{
    ...
    stub_ext::StubExt stub;
    stub.set_lamda(((QVariant (EventChannelManager::*)(const QString &, const QString &))
                   &EventChannelManager::push), [&rets, &displayMode, &sc1]
                   (EventChannelManager *, const QString &t1, const QString &t2) {
        if (t1 == "ddplugin_core") {
...
TEST(WindowFrame, onGeometryChanged)
{
    ...
    stub_ext::StubExt stub;
    stub.set_lamda(((QVariant (EventChannelManager::*)(const QString &, const QString &))
                   &EventChannelManager::push), [&rets, &sc1]
                   (EventChannelManager *, const QString &t1, const QString &t2) {
        if (t1 == "ddplugin_core") {
            if (t2 == "slot_ScreenProxy_LogicScreens") {
                return QVariant::fromValue(rets);
...
TEST(WindowFramePrivate, updateProperty)
{
    ...
    EXPECT_EQ(win->property(DesktopFrameProperty::kPropScreenAvailableGeometry).toRect()
              , screen->availableGeometry());
    EXPECT_EQ(win->property(DesktopFrameProperty::kPropScreenHandleGeometry).toRect()
              , screen->handleGeometry());
