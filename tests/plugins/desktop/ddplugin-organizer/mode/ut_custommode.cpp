...
TEST_F(UT_CustomMode, initialize)
{
    ...
    stub.set_lamda(fun_type1,[](){
        __DBG_STUB_INVOKE__
        CollectionBaseDataPtr *ptr = new CollectionBaseDataPtr(new CollectionBaseData);
        return QList{*ptr};
    });
...
TEST_F(UT_CustomMode, layout)
{
    typedef CollectionStyle(*fun_type)(const QString&);
      stub.set_lamda((fun_type)(&ConfigPresenter::customStyle),[](const QString &){
        __DBG_STUB_INVOKE__
        CollectionStyle *res = new CollectionStyle;
        return *res;
    });
    ...
    CollectionDataProvider *dataProvider = nullptr;
    CollectionHolderPointer holder(new CollectionHolder("fake", dataProvider));
...
TEST_F(UT_CustomMode, rebuild)
{
    ...
    stub.set_lamda((fun_type)&CollectionModel::files,[](){
        __DBG_STUB_INVOKE__
        QList<QUrl> *res = new QList<QUrl>;
    ...
    SurfacePointer *surfPtr = new SurfacePointer(new Surface);
...
TEST_F(UT_CustomMode, filterDropData)
{
    ...
    mode->model = new CollectionModel;
    mode->canvasViewShell = new CanvasViewShell;
    mode->canvasGridShell = new CanvasGridShell;
    mode->canvasModelShell = new CanvasModelShell;
    CollectionBaseDataPtr *ptr = new CollectionBaseDataPtr(new CollectionBaseData);
    ptr->value->items = QList{QUrl("temp")};
