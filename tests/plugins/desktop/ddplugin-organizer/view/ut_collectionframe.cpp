...
{
    CollectionFrame frame;
    QMouseEvent *event = new QMouseEvent(QEvent::Type::MouseButtonRelease, { 10, 10 },
...
{
    QPaintEvent *event1 = new QPaintEvent(QRect(1,1,2,2));
    CollectionFrame frame;

    frame.paintEvent(event1);
    EXPECT_TRUE(event1->m_accept);

    QFocusEvent *event2 = new QFocusEvent(QEvent::FocusIn);
...
protected:
    ...
    {
        frame = new CollectionFramePrivate(new CollectionFrame);
    }
