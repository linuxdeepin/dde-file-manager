...
protected:
    ...
    {
        aa = new AutoActivateWindowPrivate(new AutoActivateWindow);
...
TEST_F(UT_AutoActivateWindowPrivate, checkWindowOnX11)
{
    ...
    stub.set_lamda(& xcb_get_window_attributes,[](xcb_connection_t *c,
                   xcb_window_t      window){
        __DBG_STUB_INVOKE__
        xcb_get_window_attributes_cookie_t *cookie = new xcb_get_window_attributes_cookie_t;
        return *cookie;
    });
    ...
    stub.set_lamda(&xcb_query_tree_children,[](const xcb_query_tree_reply_t *R){
        __DBG_STUB_INVOKE__
        xcb_window_t *res =new xcb_window_t(1);
        return res;
    });
...
{
     aw->d->run = false;
     aw->d->watchedWidget = new QWidget;
