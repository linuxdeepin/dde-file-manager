...
WidgetActivceVaultFinished::WidgetActivceVaultFinished(QWidget *parent)
    : QWidget(parent)
    , m_pTips(nullptr)
    ...
    , m_pTips2(nullptr)
{
    ...
    m_pTips3 = new QLabel(tr("The setup is complete"), this);
    ...
    connect(m_pFinishedBtn, &QPushButton::clicked,
            this, &WidgetActivceVaultFinished::slotEncryptVault);
    ...
    connect(pcryfs, &VaultController::signalCreateVault,
            this, &WidgetActivceVaultFinished::slotEncryptComplete);
}

void WidgetActivceVaultFinished::slotEncryptComplete(int nState)
{
    if(nState == 0){    // 创建保险箱成功
...
void WidgetActivceVaultFinished::slotEncryptVault()
{
